...
// EnumWidgetFilter is the typed query representation for EnumWidget.
...
func (f *EnumWidgetFilter) Predicates() []predicate.EnumWidget {
	...
	entapi.AppendEach(&ps, f.ID, enumwidget.IDEQ)
	entapi.AppendEach(&ps, f.IDNEQ, enumwidget.IDNEQ)
	entapi.AppendEachSlice(&ps, f.IDIn, enumwidget.IDIn)
	entapi.AppendEachSlice(&ps, f.IDNotIn, enumwidget.IDNotIn)
	entapi.AppendEach(&ps, f.IDGT, enumwidget.IDGT)
	entapi.AppendEach(&ps, f.IDGTE, enumwidget.IDGTE)
	entapi.AppendEach(&ps, f.IDLT, enumwidget.IDLT)
	entapi.AppendEach(&ps, f.IDLTE, enumwidget.IDLTE)
	return ps
}

func parseEnumWidgetIDQueryValue(raw, whole string) (uuid.UUID, error) {
...
// ParseEnumWidgetQuery parses the complete URL query contract in sorted-key
...
func ParseEnumWidgetQuery(q url.Values) (*EnumWidgetFilter, entapi.ListRequest, error) {
	f := &EnumWidgetFilter{}
	...
	for _, key := range keys {
		...
		if recognized {
			...
			case "_q":
				err = fmt.Errorf("%w: _q value %q is invalid because EnumWidget has no Searchable fields", entapi.ErrValidation, reserved)
			}
			if err != nil {
				return nil, entapi.ListRequest{}, err
			}
		...
		case "id":
			for _, whole := range values {
				...
				switch op {
				case "eq":
					parsed, parseErr := parseEnumWidgetIDQueryValue(raw, whole)
					if parseErr != nil {
				...
				case "in":
					...
					for _, part := range parts {
						parsed, parseErr := parseEnumWidgetIDQueryValue(part, whole)
						if parseErr != nil {
							return nil, entapi.ListRequest{}, parseErr
						}
				...
				case "between":
					...
					lower, parseErr := parseEnumWidgetIDQueryValue(parts[0], whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					upper, parseErr := parseEnumWidgetIDQueryValue(parts[1], whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
...
// EnumWidgetSortKeys is the complete sort allow-list. The primary key is
// naturally Sortable; every other key requires api.Sortable().
var EnumWidgetSortKeys = []string{
	"id",
}

// EnumWidgetOrder is the single sort allow-list seam.
func EnumWidgetOrder(r entapi.ListRequest) ([]enumwidget.OrderOption, error) {
	if len(r.Sort) == 0 {
	...
	for _, spec := range r.Sort {
		by, ok := enumWidgetSortOptions[spec.Key]
		if !ok {
			return nil, fmt.Errorf("%w: cannot sort by %q; legal keys: %s", entapi.ErrValidation, spec.Key, strings.Join(EnumWidgetSortKeys, ", "))
		}
		dir := sql.OrderAsc()
		if spec.Desc {
			dir = sql.OrderDesc()
