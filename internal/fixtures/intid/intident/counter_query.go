...
func (cq *CounterQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("intident: check existence: %w", err)
	default:
		return true, nil
	}
}
...
//		GroupBy(counter.FieldLabel).
//		Aggregate(intident.Count()).
//		Scan(ctx, &v)
...
func (cq *CounterQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range cq.inters {
		if inter == nil {
			return fmt.Errorf("intident: uninitialized interceptor (forgotten import intident/runtime?)")
		}
	...
	for _, f := range cq.ctx.Fields {
		if !counter.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("intident: invalid field %q for query", f)}
		}
	}
	if cq.path != nil {
