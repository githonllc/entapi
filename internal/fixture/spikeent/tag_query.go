...
func (tq *TagQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("spikeent: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (tq *TagQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range tq.inters {
		if inter == nil {
			return fmt.Errorf("spikeent: uninitialized interceptor (forgotten import spikeent/runtime?)")
		}
	...
	for _, f := range tq.ctx.Fields {
		if !tag.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("spikeent: invalid field %q for query", f)}
		}
	}
	if tq.path != nil {
