...
// ============================================================================
// Query surface for NamedTypeWidget — filters, free-text search and the sort
...
// NamedTypeWidgetFilter is the structured query surface for NamedTypeWidget.
//
...
// namedTypeWidgetSortOptions maps an allowed sort key to ent's own order
// builder for that column. It is unexported: the mapping is reached through
// NamedTypeWidgetOrder, which checks the key first.
var namedTypeWidgetSortOptions = map[string]func(...sql.OrderTermOption) namedtypewidget.OrderOption{}

// NamedTypeWidgetSortKeys is the sort allow-list: exactly the fields annotated
...
// NamedTypeWidgetOrder turns a list request into ent order options.
//
// The caller's string is checked against NamedTypeWidgetSortKeys and then thrown
...
func NamedTypeWidgetOrder(r entdomain.ListRequest) ([]namedtypewidget.OrderOption, error) {
	key, desc, err := r.SortKey(NamedTypeWidgetSortKeys, "")
	if err != nil {
		return nil, err
	}
	by, ok := namedTypeWidgetSortOptions[key]
	if !ok {
		// Only reachable when no sort was requested and no default applies.
		return nil, nil
	}
	if desc {
		return []namedtypewidget.OrderOption{by(sql.OrderDesc())}, nil
	}
	return []namedtypewidget.OrderOption{by(sql.OrderAsc())}, nil
}
