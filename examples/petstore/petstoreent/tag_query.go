...
func (tq *TagQuery) QueryPets() *PetQuery {
...
func (tq *TagQuery) WithPets(opts ...func(*PetQuery)) *TagQuery {
	query := (&PetClient{config: tq.config}).Query()
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withPets; query != nil {
		if err := tq.loadPets(ctx, query, nodes,
			func(n *Tag) { n.Edges.Pets = []*Pet{} },
			func(n *Tag, e *Pet) { n.Edges.Pets = append(n.Edges.Pets, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadPets(ctx context.Context, query *PetQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Pet)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.PetsTable)
		s.Join(joinT).On(s.C(pet.FieldID), joinT.C(tag.PetsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.PetsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.PetsPrimaryKey[1]))
