...
func (tu *TodoUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if tu.mutation.PriorityCleared() {
		_spec.ClearField(todo.FieldPriority, field.TypeInt)
	}
...
func (tuo *TodoUpdateOne) sqlSave(ctx context.Context) (_node *Todo, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`todoent: missing "Todo.id" for update`)}
	}
	...
	if fields := tuo.fields; len(fields) > 0 {
		...
		for _, f := range fields {
			if !todo.ValidColumn(f) {
				return nil, &ValidationError{Name: f, err: fmt.Errorf("todoent: invalid field %q for query", f)}
			}
			if f != todo.FieldID {
