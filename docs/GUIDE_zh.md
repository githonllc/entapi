...
已针对 **ogen v1.24.0** 验证，而且验的是行为、不是"它能解析"：生成的客户端可编译，
...
`OpGet`、`OpPatch`、`OpDelete`，不属于任何资源的端点则是 `OpNone`。它们把端点的身份作为
数据带上，于是「按受众切分这棵树」变成一次编译器能检查的比较，而不是对路径文本做匹配
...
这五个之上还有一件导出物，而这条分界真正被强制的地方就在那里。`ParseFieldValues`
...
> `OpKindValue`/`OpKindList`/`OpKindFlag`/`OpKindRange`、`findQueryOp`；
...
第二种不依赖读取器也会咬人：`Has<Field>()` 同样生成在原始请求上，而 `has_x` 在那里正是一个
同名的结构体字段，所以这一对两条路都编译不过。两条消息都指向 `.StorageKey(…)`，因为 JSON tag
...
1. **首行**（读文件头 4096 字节后按第一个换行截断）包含
   ...
   extension` —— 两种拼写都归本扩展所有，下面那条逃生口对两者完全一样，且
2. 不是本轮写入的。

这就是「一次 schema 编辑不会弄红你的构建」的由来：删掉一个实体，它的
`_dto.go`/`_filter.go`/`_wiring.go`/`_handler.go` 会随之而去，而不是作为「引用一个 ent 并不
生成的 builder」的残骸留下来。判据是 marker 而不是一份名字清单，所以任何带 marker、本扩展
当前不再产出的文件，都走同一条路。
...
- **没有乐观锁，丢失更新是已知边界。** 生成的 patch wiring 是
  ...
  版本列，而按名猜 `version`/`updated_at` 正是本包在别处一律拒绝的那类约定推导。出路是
  定制点：
