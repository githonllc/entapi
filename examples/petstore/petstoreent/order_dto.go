...
// - transport → order_wiring.go → entapi runtime → ent.Client
...
// OrderCreateRequest is the create request for Order.
//
...
// orderCreateRequestTags is the canonical JSON key of every field on
// OrderCreateRequest, in declaration order. UnmarshalJSON needs the tags as
// data, not as struct tags, to tell a case variant from an unrelated key.
var orderCreateRequestTags = []string{"quantity", "status", "ship_date", "pet_id"}

// OrderCreateRequestTags returns the canonical JSON key of every field on
// OrderCreateRequest, in declaration order. Generated handlers compare raw
...
func OrderCreateRequestTags() []string {
	return append([]string(nil), orderCreateRequestTags...)
}
...
func (r *OrderCreateRequest) UnmarshalJSON(b []byte) error {
	type alias OrderCreateRequest // breaks the UnmarshalJSON recursion
	var raw map[string]json.RawMessage
	if err := json.Unmarshal(b, &raw); err != nil {
		return err
	}
	for k := range raw {
		for _, t := range orderCreateRequestTags {
...
// everything.
func (r *OrderCreateRequest) has(field string) bool {
	if r.present == nil {
		return true
	}
...
// HasQuantity reports whether the payload carried "quantity".
func (r *OrderCreateRequest) HasQuantity() bool { return r.has("quantity") }

// HasStatus reports whether the payload carried "status".
func (r *OrderCreateRequest) HasStatus() bool { return r.has("status") }

// HasShipDate reports whether the payload carried "ship_date".
func (r *OrderCreateRequest) HasShipDate() bool { return r.has("ship_date") }

// HasPetID reports whether the payload carried "pet_id".
func (r *OrderCreateRequest) HasPetID() bool { return r.has("pet_id") }

// ValidOrderCreateRequest wraps a OrderCreateRequest that has
...
type ValidOrderCreateRequest struct{ r *OrderCreateRequest }

// HasQuantity reports whether the payload carried "quantity".
func (v *ValidOrderCreateRequest) HasQuantity() bool { return v.r.HasQuantity() }

// HasStatus reports whether the payload carried "status".
func (v *ValidOrderCreateRequest) HasStatus() bool { return v.r.HasStatus() }

// HasShipDate reports whether the payload carried "ship_date".
func (v *ValidOrderCreateRequest) HasShipDate() bool { return v.r.HasShipDate() }

// HasPetID reports whether the payload carried "pet_id".
func (v *ValidOrderCreateRequest) HasPetID() bool { return v.r.HasPetID() }

// Validate checks the request and returns the only type Apply accepts.
func (r *OrderCreateRequest) Validate() (*ValidOrderCreateRequest, error) {
	if r == nil {
		return nil, fmt.Errorf("%w: create request is nil", entapi.ErrValidation)
	}
	if !r.HasQuantity() {
		return nil, fmt.Errorf("%w: quantity is required", entapi.ErrValidation)
	}
	if !r.HasPetID() {
		return nil, fmt.Errorf("%w: pet_id is required", entapi.ErrValidation)
	}
	if r.Status != nil {
		if err := order.StatusValidator(*r.Status); err != nil {
			return nil, fmt.Errorf("%w: %v", entapi.ErrValidation, err)
		}
	}
	return &ValidOrderCreateRequest{r: r}, nil
}
...
func (v *ValidOrderCreateRequest) Apply(b *OrderCreate) *OrderCreate {
	r := v.r
	...
	if r.ShipDate != nil {
		b.SetShipDate(*r.ShipDate)
	}
	b.SetPetID(r.PetID)
	return b
}

// OrderPatchRequest is the partial-update request for Order.
//
...
// orderPatchRequestTags is the canonical JSON key of every field on
// OrderPatchRequest, in declaration order.
var orderPatchRequestTags = []string{"quantity", "status", "ship_date", "pet_id"}

// OrderPatchRequestTags returns the canonical JSON key of every field on
// OrderPatchRequest, in declaration order. Generated handlers compare raw
// request keys against this data to reject unknown keys.
//
// Like Endpoints(), it returns a fresh copy on every call: slices are mutable, so
// exporting the variable would let any consumer mutate validation data that
...
// the source of truth; this accessor is for consumers that decline the generated
// handler and write their own bind step.
func OrderPatchRequestTags() []string {
	return append([]string(nil), orderPatchRequestTags...)
}
...
func (r *OrderPatchRequest) UnmarshalJSON(b []byte) error {
	type alias OrderPatchRequest // breaks the UnmarshalJSON recursion
	var raw map[string]json.RawMessage
	if err := json.Unmarshal(b, &raw); err != nil {
		return err
	}
	for k := range raw {
		for _, t := range orderPatchRequestTags {
			if k != t && strings.EqualFold(k, t) {
				return fmt.Errorf("%w: unknown key %q (did you mean %q?)", entapi.ErrValidation, k, t)
			}
		}
...
// nothing for it.
func (r *OrderPatchRequest) has(field string) bool { return r.present[field] }

// HasQuantity reports whether the payload carried "quantity".
func (r *OrderPatchRequest) HasQuantity() bool { return r.has("quantity") }

// HasStatus reports whether the payload carried "status".
func (r *OrderPatchRequest) HasStatus() bool { return r.has("status") }

// HasShipDate reports whether the payload carried "ship_date".
func (r *OrderPatchRequest) HasShipDate() bool { return r.has("ship_date") }

// HasPetID reports whether the payload carried "pet_id".
func (r *OrderPatchRequest) HasPetID() bool { return r.has("pet_id") }

// ValidOrderPatchRequest wraps a OrderPatchRequest that has passed
// Validate. It is the only type Apply is defined on.
type ValidOrderPatchRequest struct{ r *OrderPatchRequest }
...
func (v *ValidOrderPatchRequest) HasQuantity() bool { return v.r.HasQuantity() }

// Quantity reports the value the payload carried for
// "quantity", and whether Apply will Set it.
//
// Presence is two of the three states a PATCH field has; this reads the third.
// Paired with HasQuantity(), which is "present" below:
//
...
func (v *ValidOrderPatchRequest) Quantity() (int, bool) {
	if !v.r.HasQuantity() || v.r.Quantity == nil {
		var zero int
		return zero, false
	}
	return *v.r.Quantity, true
}
...
// from "leave it alone" — is unreachable from the customization point that is
// supposed to act on it.
func (v *ValidOrderPatchRequest) HasStatus() bool { return v.r.HasStatus() }
...
// the customization point is forced to allocate an update builder it never
// executes, Apply the request to it and read back Mutation() (#113).
func (v *ValidOrderPatchRequest) Status() (order.Status, bool) {
...
// from "leave it alone" — is unreachable from the customization point that is
// supposed to act on it.
func (v *ValidOrderPatchRequest) HasShipDate() bool { return v.r.HasShipDate() }

// ShipDate reports the value the payload carried for
// "ship_date", and whether Apply will Set it.
//
// Presence is two of the three states a PATCH field has; this reads the third.
// Paired with HasShipDate(), which is "present" below:
//
//	ok                    the payload carried a value; Apply will Set it
//	!ok, present          the payload carried an explicit null; Apply will
//	                      Clear it. Reachable only for a clearable field —
...
// the customization point is forced to allocate an update builder it never
// executes, Apply the request to it and read back Mutation() (#113).
func (v *ValidOrderPatchRequest) ShipDate() (time.Time, bool) {
	if !v.r.HasShipDate() || v.r.ShipDate == nil {
		var zero time.Time
		return zero, false
	}
	return *v.r.ShipDate, true
}
...
// from "leave it alone" — is unreachable from the customization point that is
// supposed to act on it.
func (v *ValidOrderPatchRequest) HasPetID() bool { return v.r.HasPetID() }

// PetID reports the value the payload carried for
// "pet_id", and whether Apply will Set it.
//
// Presence is two of the three states a PATCH field has; this reads the third.
// Paired with HasPetID(), which is "present" below:
//
//	ok                    the payload carried a value; Apply will Set it
//	!ok, present          the payload carried an explicit null; Apply will
//	                      Clear it. Reachable only for a clearable field —
...
// the customization point is forced to allocate an update builder it never
// executes, Apply the request to it and read back Mutation() (#113).
func (v *ValidOrderPatchRequest) PetID() (int, bool) {
	if !v.r.HasPetID() || v.r.PetID == nil {
		var zero int
		return zero, false
	}
	return *v.r.PetID, true
}
...
func (r *OrderPatchRequest) Validate() (*ValidOrderPatchRequest, error) {
	if r == nil {
		return nil, fmt.Errorf("%w: patch request is nil", entapi.ErrValidation)
	}
	if r.HasQuantity() && r.Quantity == nil {
		return nil, fmt.Errorf("%w: quantity cannot be null", entapi.ErrValidation)
	}
	if r.HasStatus() && r.Status == nil {
		return nil, fmt.Errorf("%w: status cannot be null", entapi.ErrValidation)
	}
	if r.HasPetID() && r.PetID == nil {
		return nil, fmt.Errorf("%w: pet_id cannot be null", entapi.ErrValidation)
	}
	if r.Status != nil {
		if err := order.StatusValidator(*r.Status); err != nil {
			return nil, fmt.Errorf("%w: %v", entapi.ErrValidation, err)
	...
	return &ValidOrderPatchRequest{r: r}, nil
}
...
func (v *ValidOrderPatchRequest) Apply(b *OrderUpdateOne) *OrderUpdateOne {
	r := v.r
	if r.HasQuantity() {
		b.SetQuantity(*r.Quantity)
	}
...
// OrderSummary is the shape Order takes on another entity's response.
//
// It carries the same scalar fields as OrderResponse and no edges at all.
// That is what bounds expansion: NewOrderResponse calls summary
...
// NewOrderSummary converts an entity to its summary DTO. It cannot fail:
// a summary reads no edges.
func NewOrderSummary(e *Order) *OrderSummary {
	if e == nil {
		return nil
	}
	return &OrderSummary{
...
// OrderResponse represents the response for Order.
//
...
// OrderListResponse below refers to this type either way.
//
...
// NewOrderResponse converts an entity to its response DTO.
//
...
// reads as "this order has none".
//
// That error is cheap because OrderQueryWithResponseEdges below is
...
func NewOrderResponse(e *Order) (*OrderResponse, error) {
	if e == nil {
		return nil, nil
	}
	r := &OrderResponse{
	...
	petEdge, err := e.Edges.PetOrErr()
	switch {
	case err == nil:
		r.Pet = NewPetSummary(petEdge)
	case IsNotFound(err):
...
// OrderQueryWithResponseEdges applies the eager-load plan for OrderResponse.
//
...
// NewOrderResponse return a not-loaded-edge error at run time. It fails
// loudly; it never silently narrows the response.
//
// OrderClient.Get loads no edges and therefore cannot serve this response
// type. Wiring for Order must go through Query with this plan applied.
func OrderQueryWithResponseEdges(q *OrderQuery) *OrderQuery {
	return q.WithPet()
}

// OrderListResponse is the NAMED, non-generic shape of one list page —
// for OpenAPI/swaggo-class annotation tooling, which cannot express the
// generic entapi.Page[OrderResponse] that ListOrders returns.
// Convert at the handler boundary with NewOrderListResponse.
...
// NewOrderListResponse converts the page ListOrders returns into
// the named list shape. The conversion expression is the shape contract:
// if OrderListResponse and entapi.Page ever diverge in field set,
...
func NewOrderListResponse(p *entapi.Page[OrderResponse]) *OrderListResponse {
	if p == nil {
		return nil
	}
	r := OrderListResponse(*p)
	return &r
}
