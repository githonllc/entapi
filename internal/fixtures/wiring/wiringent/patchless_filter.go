...
// ============================================================================
// Query surface for Patchless — filters, free-text search and the sort
...
// PatchlessFilter is the structured query surface for Patchless.
//
...
// patchlessSortOptions maps an allowed sort key to ent's own order
// builder for that column. It is unexported: the mapping is reached through
// PatchlessOrder, which checks the key first.
var patchlessSortOptions = map[string]func(...sql.OrderTermOption) patchless.OrderOption{}

// PatchlessSortKeys is the sort allow-list: exactly the fields annotated
...
// PatchlessOrder turns a list request into ent order options.
//
// The caller's string is checked against PatchlessSortKeys and then thrown
...
func PatchlessOrder(r entapi.ListRequest) ([]patchless.OrderOption, error) {
	key, desc, err := r.SortKey(PatchlessSortKeys, "")
	if err != nil {
		return nil, err
	}
	dir := sql.OrderAsc()
	...
	by, ok := patchlessSortOptions[key]
	if !ok {
		return []patchless.OrderOption{patchless.ByID(sql.OrderAsc())}, nil
	}
	order := []patchless.OrderOption{by(dir)}
	if key != "id" {
		order = append(order, patchless.ByID(dir))
	}
	return order, nil
}
