...
func (oq *OrderQuery) QueryPet() *PetQuery {
	query := (&PetClient{config: oq.config}).Query()
...
func (oq *OrderQuery) WithPet(opts ...func(*PetQuery)) *OrderQuery {
	query := (&PetClient{config: oq.config}).Query()
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := oq.withPet; query != nil {
		if err := oq.loadPet(ctx, query, nodes, nil,
			func(n *Order, e *Pet) { n.Edges.Pet = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrderQuery) loadPet(ctx context.Context, query *PetQuery, nodes []*Order, init func(*Order), assign func(*Order, *Pet)) error {
...
func (oq *OrderQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := oq.ctx.Fields; len(fields) > 0 {
		...
		if oq.withPet != nil {
			_spec.Node.AddColumnOnce(order.FieldPetID)
		}
	}
