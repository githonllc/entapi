...
func (oq *OrderQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("collideent: check existence: %w", err)
	default:
		return true, nil
	}
}
...
//
//	client.Order.Query().
//		GroupBy(order.FieldReference).
//		Aggregate(collideent.Count()).
//		Scan(ctx, &v)
...
func (oq *OrderQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range oq.inters {
		if inter == nil {
			return fmt.Errorf("collideent: uninitialized interceptor (forgotten import collideent/runtime?)")
		}
	...
	for _, f := range oq.ctx.Fields {
		if !order.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("collideent: invalid field %q for query", f)}
		}
	}
	if oq.path != nil {
