...
func (pq *PetQuery) QueryTags() *TagQuery {
...
func (pq *PetQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("petstoreent: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (pq *PetQuery) WithTags(opts ...func(*TagQuery)) *PetQuery {
	query := (&TagClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PetQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range pq.inters {
		if inter == nil {
			return fmt.Errorf("petstoreent: uninitialized interceptor (forgotten import petstoreent/runtime?)")
		}
	...
	for _, f := range pq.ctx.Fields {
		if !pet.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("petstoreent: invalid field %q for query", f)}
		}
	}
	if pq.path != nil {
...
func (pq *PetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Pet, error) {
	...
	if query := pq.withCategory; query != nil {
		if err := pq.loadCategory(ctx, query, nodes, nil,
			func(n *Pet, e *Category) { n.Edges.Category = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withTags; query != nil {
		if err := pq.loadTags(ctx, query, nodes,
			func(n *Pet) { n.Edges.Tags = []*Tag{} },
			func(n *Pet, e *Tag) { n.Edges.Tags = append(n.Edges.Tags, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PetQuery) loadCategory(ctx context.Context, query *CategoryQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *Category)) error {
...
func (pq *PetQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *Tag)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(pet.TagsTable)
		s.Join(joinT).On(s.C(tag.FieldID), joinT.C(pet.TagsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(pet.TagsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(pet.TagsPrimaryKey[0]))
...
func (pq *PetQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withCategory != nil {
			_spec.Node.AddColumnOnce(pet.FieldCategoryID)
		}
	}
