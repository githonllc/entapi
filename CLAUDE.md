...
- `templateFuncMap()` layers three sources, later wins: Ent's `gen.Funcs` → this package's `templateFuncs()` → the configuration closures `entapiPkg`, `openapiTitle` and `openapiVersion`. Closures are exempt from `TestTemplateInvocationsAreRegistered`'s reverse direction (it reads `templateFuncs()` only), so a genuine helper belongs in `templateFuncs()` and only configuration belongs here.
