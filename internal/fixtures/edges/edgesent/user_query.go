...
// QueryReviewed chains the current query on the "reviewed" edge.
func (uq *UserQuery) QueryReviewed() *PostQuery {
	query := (&PostClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (uq *UserQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("edgesent: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withReviewed: uq.withReviewed.Clone(),
...
// WithReviewed tells the query-builder to eager-load the nodes that are connected to
// the "reviewed" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithReviewed(opts ...func(*PostQuery)) *UserQuery {
	query := (&PostClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withReviewed = query
	return uq
}
...
func (uq *UserQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range uq.inters {
		if inter == nil {
			return fmt.Errorf("edgesent: uninitialized interceptor (forgotten import edgesent/runtime?)")
		}
	...
	for _, f := range uq.ctx.Fields {
		if !user.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("edgesent: invalid field %q for query", f)}
		}
	}
	if uq.path != nil {
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withReviewed; query != nil {
		if err := uq.loadReviewed(ctx, query, nodes,
			func(n *User) { n.Edges.Reviewed = []*Post{} },
			func(n *User, e *Post) { n.Edges.Reviewed = append(n.Edges.Reviewed, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadReviewed(ctx context.Context, query *PostQuery, nodes []*User, init func(*User), assign func(*User, *Post)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(post.FieldReviewerID)
	}
	query.Where(predicate.Post(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ReviewedColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
