...
a DO NOT EDIT header. Regenerate; never hand-edit. The one exception is
`stale/ent/trinket_dto.go`, which is hand-written on purpose and says so.
...
`basic` and `fieldshapes` are also the corpus for
