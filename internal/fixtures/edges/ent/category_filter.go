...
// ============================================================================
// Query surface for Category — filters, free-text search and the sort
...
// CategoryFilter is the structured query surface for Category.
//
...
// categorySortOptions maps an allowed sort key to ent's own order
// builder for that column. It is unexported: the mapping is reached through
// CategoryOrder, which checks the key first.
var categorySortOptions = map[string]func(...sql.OrderTermOption) category.OrderOption{}

// CategorySortKeys is the sort allow-list: exactly the fields annotated
...
// CategoryOrder turns a list request into ent order options.
//
// The caller's string is checked against CategorySortKeys and then thrown
...
func CategoryOrder(r entdomain.ListRequest) ([]category.OrderOption, error) {
	key, desc, err := r.SortKey(CategorySortKeys, "")
	if err != nil {
		return nil, err
	}
	by, ok := categorySortOptions[key]
	if !ok {
		// Only reachable when no sort was requested and no default applies.
		return nil, nil
	}
	if desc {
		return []category.OrderOption{by(sql.OrderDesc())}, nil
	}
	return []category.OrderOption{by(sql.OrderAsc())}, nil
}
