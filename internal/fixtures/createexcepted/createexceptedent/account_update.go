...
func (auo *AccountUpdateOne) sqlSave(ctx context.Context) (_node *Account, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`createexceptedent: missing "Account.id" for update`)}
	}
	...
	if fields := auo.fields; len(fields) > 0 {
		...
		for _, f := range fields {
			if !account.ValidColumn(f) {
				return nil, &ValidationError{Name: f, err: fmt.Errorf("createexceptedent: invalid field %q for query", f)}
			}
