...
完整参考。[`README.md`](../README_zh.md) 是入口：这个扩展是什么、怎么跑起一个例子、
以及最常用的那几张表。本文件是它每一节的长版——每条规则是什么、为什么是这条规则，
以及指向让它成立的那段源码的 `> **实现：**` 指针。

关于本模块**移除过**的符号，这里不写。迁移注记在
[`README_zh.md`](../README_zh.md#迁移注记)，包注释里的指针也指向那儿。
...
- [生成了什么](#生成了什么)
- [生成的 HTTP](#生成的-http) —— 自定义实现、OpenAPI 文档、ogen、端点注册、自己写 handler
- [请求：三态存在性](#请求三态存在性)
- [响应、摘要与边](#响应摘要与边)
- [查询面](#查询面) —— 过滤、全文、排序、分页
- [接线与错误映射](#接线与错误映射)
...
- [字段形态](#字段形态) · [注解表面](#注解表面) · [限制](#限制)
- [与 DESIGN-v2 的偏离](#与-design-v2-的偏离) · [与 DESIGN-v3 的偏离](#与-design-v3-的偏离)
...
这个切分是承重的，不是整洁癖：根包用 `//go:embed` 内嵌十个模板，并在**包初始化时**把十份
...
> `softDeleteConfigInitTemplate`（十个包级 `var`，全部在 init 时求值）；
...
仓库里有两份可用的 `entc.go`，每份旁边都有一个 `generate.go` 放着真正触发它的
`//go:generate go run -mod=mod entc.go`：
[`examples/todo/todoent/entc.go`](../examples/todo/todoent/entc.go) 是最小版，
[`examples/petstore/petstoreent/entc.go`](../examples/petstore/petstoreent/entc.go)
额外设置了 OpenAPI 的 title 与 version。忘掉 `generate.go` 是最常见的第一个坑：
`go generate ./...` 会静默地什么都不做。
...
> **实现：** `api/annotations.go` — `ResourceAnnotation`、`FieldAnnotation`、
> `EdgeAnnotation` 及它们的 `Merge` 方法；`funcs_scope.go` —
> `getResourceAnnotation`、`getFieldAnnotation`、`getEdgeAnnotation`（三个归一化
> 读取器），底下是 `decodeResourceAnnotation` / `decodeAnnotation`（那次 JSON 往返），
> 外加 `isResource`、`resourceExcepts`、`hasCreateFamily` 与 `blockedCreateFields`
> ——最后这个正是判定「create 一族可证明无法工作」的那一段；
...
> **实现：** `templates/http.tmpl` — `APIHandler`、`API`、`With`、`ServeHTTP`、
> `Mount`、`Endpoints`、每操作一个的 `…Endpoint()` accessor 与 `OpenAPIEndpoint()`；
> `templates/handler.tmpl` — bind → call → write 的函数体与 `{Op}{Entity}Fn` 类型；
> `templates/openapi.tmpl` 与 `templates/openapi_embed.tmpl`；`funcs_http.go` —
> `resourceOperation`、`resourceOps`、`routePath`、`idParseExpr`、`handlerImports`、
> `entSnake` / `mustEntSnake`；`funcs_openapi.go` — `yamlQuote`、`openapiSchema`、
...
> `Endpoint`、`Op` 及 `OpNone`/`OpList`/`OpCreate`/`OpGet`/`OpPatch`/`OpDelete`、
...
> `classifyError`；生成物样例：
> `internal/fixtures/httpdemo/httpdemoent/entapi_http.go` 及其 `openapi.yaml`；
> 行为证明：`internal/fixtures/httpdemo/e2e`
...
### 这条分界，与 runtime 导出的原语

分工本身才是承重的那部分：**runtime 只拥有词法语法，生成的代码拥有语义许可。** runtime
无从知道某个字段允许哪些操作符——那是 `$field.Ops` 与线格式词汇表的交集，在生成期就定
下来了——所以它只导出词法那一半，不多一点：

| runtime 函数 | 回答什么 |
|---|---|
| `SplitOp(value)` | 在第一个冒号处切分，返回 `(op, operand, explicit)`；没有冒号则 `explicit == false`，整个值就是操作数 |
| `KnownQueryOperator(op)` | `op` 到底在不在全局线格式词汇表里——就是上表那十七个前缀。**不**检查每字段许可 |
| `ReservedQueryValue(key, values)` | `key` 是不是那四个 `_` 前缀参数之一，以及它是否单值 |
| `ParsePageParam(name, value)` | 一索引的正整数 `_page`/`_size`，其中 `_size=0` 会被点名拒绝 |
| `ParseSortSpecs(value)` | 把 `_sort` 解析成 `[]SortSpec`；方向必须是 `asc` 或 `desc`。**不**检查 key 许可——生成的 `{Entity}Order` 才是唯一的白名单接缝 |

其中两个单独用是刻意不完整的，这是设计而非疏漏：`KnownQueryOperator` 只说明某个前缀在词汇
表里存在过，`ParseSortSpecs` 接受任何 key。一个手写 parser 的消费者白拿到词法层，仍然必须
自己提供白名单——而那正是 `Parse{Entity}Query` 与 `{Entity}Order` 的职责。

这五个之上还有一件导出物，而这条分界真正被强制的地方就在那里（#143）。`ParseFieldValues`
为每个字段各跑一次逐字段分发循环，而它拿到的是一张**由生成代码构造的表**：
...
所有与字段相关的东西都以参数形式送进来——`ops` 是这个字段允许的操作符行，`parse` 是它的
转换函数，`legal` 是错误信息里逐字引用的预拼接合法操作符列表，`strict` 则是生成期就冻结
下来的 `WithStrictQueryOperators()`。runtime 留下的只有每个字段共享的那部分：操作符切分、
`eq` 回退、逗号切分，以及把每次失败都包装成 `ErrValidation`。

一行 `QueryOp[T]` 携带 `Prefix`、`OpKind`，以及**指向生成 filter 结构体的强类型指针**——
`One`、`Second`、`Many`、`Null`——所以分发既不用反射，也不做以字符串为键的谓词目标查找，
而缺失或类型不对的 filter 字段会在生成包里变成编译错误。`OpKind` 恰好四种形态：

| `OpKind` | 操作数 | 追加到 |
|---|---|---|
| `OpKindValue` | 一个值 | `One` |
| `OpKindList` | 逗号分隔，逐个解析 | `Many`，作为一个条目 |
| `OpKindFlag` | 必须为**空**（`is_null:` / `not_null:`） | `Null`，写入该行的 `BoolValue` |
| `OpKindRange` | 恰好两个逗号分隔的部分（`between:`） | 下界进 `One`，上界进 `Second` |

一条诚实的残留，写在该类型自己的注释里：`Kind` 与「哪个槽位指针被设置」之间的配对，是
`filter.tmpl` 作为唯一构造点维护的契约，**不是**编译器检查的东西。`ParseFieldValues` 只能
抓到落在四种之外的 `Kind`——并且报成普通错误而非校验失败——而一行 `Kind` 合法、但对应槽位
为 nil 的表，会在 nil 解引用处 panic。

> **实现：** `funcs_filter.go` — `queryFields`、`parseFields`、`searchFields`、
> `unfilterableFields`、`isSortable`、`fieldParseOperators`、
> `fieldParseConversion`、`opWirePrefix`、`substringOps`、`filterImports`；
> `runtime/types.go` — `ListRequest`、`SortSpec`、
> `DefaultPageSize`（20）、`MaxPageSize`（1000）；`runtime/urlquery.go` —
> `queryOperators`（十七项词汇表）、`reservedQueryParams`（四项）、`SplitOp`、
> `KnownQueryOperator`、`ReservedQueryValue`、`ParsePageParam`、`ParseSortSpecs`；
> `runtime/queryops.go` — `ParseFieldValues`、`QueryOp`、`OpKind` 及
> `OpKindValue`/`OpKindList`/`OpKindFlag`/`OpKindRange`、`findQueryOp`（#143）；
...
图级保留名有四个：`ErrorMap`，以及 `API`、`APIHandler`、`APIOption` 三者——后三个是因为
`entapi_http.go` 把它们全部声明出来，所以叫其中任何一个的实体，撞得和叫 handler 类型的
实体一样狠。每一个都各自门控在「它那份文件真的会被写出来」的条件上，这也是这项检查放在
图级而不是节点循环里的原因。一个名为 `ErrorMap` 的实体会让 ent 发射 `type ErrorMap`，而 `entapi_errors.go` 发射
...
> **实现：** `schema_conflicts.go` — `checkGraphConflicts`（图级入口）、
> `nodeConflicts`（每节点分发器，空 PATCH 检查也内联在它里面，没有单独的具名函数）、
...
交换，而那在跨平台上并不原子。（[ADR-0003](adr/0003-per-run-atomic-generation.md)）

**清理会删除陈旧文件，并以一个 marker 判定所有权。** 一次成功的生成之后（**只在成功之后**
——失败的一轮什么都不删），生成器扫描目标目录，删除任何满足以下**两条**的 `.go` 文件：

1. **首行**（读文件头 4096 字节后按第一个换行截断）包含
   `Code generated by entapi extension` **或** `Code generated by entdomain
   extension`，且
2. 不是本轮写入的。

**两种 marker 拼写都被识别，而且是永久的。** `entdomain` 是本 module 改名之前这个扩展写下
的那一种（ADR-0011），识别它不是一个过渡窗口：去掉它会让每一棵改名前的消费者代码树留下一
堆陈旧文件，构建随后被它们绊倒，而留着它只花一次字符串比较。下面那条逃生口对两种拼写完全
一样——删掉 marker 那一行，文件就是你的。
...
> `writeFormatted`、`pendingFile`；`cleanup.go` — `generatedMarker`、
> `legacyGeneratedMarker`、`markerScanBytes`、`removeStaleArtifacts`、
> `isCleanupCandidate`、`removeIfStale`、`hasGeneratedMarker`，以及五个图级文件名
> 常量（`softDeleteFileName`、`errorMapFileName`、`httpFileName`、
> `openapiFileName`、`openapiEmbedFileName`）
...
那些最安静地伤害你的条目，按危害的安静程度排序，在
[`README_zh.md`](../README_zh.md#陷阱)。它们放在那边而不是这边，是因为那是唯一一份
读者必须在采用**之前**看到的清单，而不是之后。
...
[`docs/DESIGN-v2.md`](DESIGN-v2.md) 的抬头写着「实现未开始」，那是**陈旧的**：它提出的
...
[`docs/DESIGN-v3-final.md`](DESIGN-v3-final.md) 的抬头同样写着「实现未开始」，
...
本模块移除过的每一个符号，以及随之而来的线格式变更，都在
[`README_zh.md`](../README_zh.md#迁移注记)——只有一份而不是两份，因为告诉读者去哪儿找的
那些包注释指向那里。
