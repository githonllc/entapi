...
func (wq *WidgetQuery) Order(o ...widget.OrderOption) *WidgetQuery {
...
func (wq *WidgetQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (wq *WidgetQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
// Returns a *NotSingularError when more than one Widget entity is found.
...
// Returns a *NotSingularError when more than one Widget ID is found.
// Returns a *NotFoundError when no entities are found.
func (wq *WidgetQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (wq *WidgetQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (wq *WidgetQuery) All(ctx context.Context) ([]*Widget, error) {
	...
	qr := querierAll[[]*Widget, *WidgetQuery]()
	return withInterceptors[[]*Widget](ctx, wq, qr, wq.inters)
}
...
func (wq *WidgetQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (wq *WidgetQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := wq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (wq *WidgetQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, wq, querierCount[*WidgetQuery](), wq.inters)
}
...
func (wq *WidgetQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("basicent: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (wq *WidgetQuery) Clone() *WidgetQuery {
	...
	return &WidgetQuery{
		config:     wq.config,
		ctx:        wq.ctx.Clone(),
		order:      append([]widget.OrderOption{}, wq.order...),
...
func (wq *WidgetQuery) Select(fields ...string) *WidgetSelect {
	...
	sbuild := &WidgetSelect{WidgetQuery: wq}
	sbuild.label = widget.Label
	sbuild.flds, sbuild.scan = &wq.ctx.Fields, sbuild.Scan
	return sbuild
}

// Aggregate returns a WidgetSelect configured with the given aggregations.
func (wq *WidgetQuery) Aggregate(fns ...AggregateFunc) *WidgetSelect {
...
func (wq *WidgetQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range wq.inters {
		if inter == nil {
			return fmt.Errorf("basicent: uninitialized interceptor (forgotten import basicent/runtime?)")
		}
	...
	for _, f := range wq.ctx.Fields {
		if !widget.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("basicent: invalid field %q for query", f)}
		}
	}
	if wq.path != nil {
...
func (wq *WidgetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Widget, error) {
	var (
		nodes = []*Widget{}
		_spec = wq.querySpec()
	)
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*Widget).scanValues(nil, columns)
	}
...
func (wq *WidgetQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(widget.Table, widget.Columns, sqlgraph.NewFieldSpec(widget.FieldID, field.TypeUUID))
	_spec.From = wq.sql
...
func (wgb *WidgetGroupBy) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, wgb.build.ctx, ent.OpQueryGroupBy)
	if err := wgb.build.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*WidgetQuery, *WidgetGroupBy](ctx, wgb.build, wgb, wgb.build.inters, v)
}

func (wgb *WidgetGroupBy) sqlScan(ctx context.Context, root *WidgetQuery, v any) error {
...
func (ws *WidgetSelect) Aggregate(fns ...AggregateFunc) *WidgetSelect {
	ws.fns = append(ws.fns, fns...)
	return ws
}

// Scan applies the selector query and scans the result into the given value.
func (ws *WidgetSelect) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, ws.ctx, ent.OpQuerySelect)
	if err := ws.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*WidgetQuery, *WidgetSelect](ctx, ws.WidgetQuery, ws, ws.inters, v)
}

func (ws *WidgetSelect) sqlScan(ctx context.Context, root *WidgetQuery, v any) error {
