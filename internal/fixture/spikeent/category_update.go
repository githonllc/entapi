...
func (cuo *CategoryUpdateOne) sqlSave(ctx context.Context) (_node *Category, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`spikeent: missing "Category.id" for update`)}
	}
	...
	if fields := cuo.fields; len(fields) > 0 {
		...
		for _, f := range fields {
			if !category.ValidColumn(f) {
				return nil, &ValidationError{Name: f, err: fmt.Errorf("spikeent: invalid field %q for query", f)}
			}
