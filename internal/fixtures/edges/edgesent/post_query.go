...
func (pq *PostQuery) QueryReviewer() *UserQuery {
	query := (&UserClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (pq *PostQuery) Clone() *PostQuery {
	...
	return &PostQuery{
		...
		withReviewer: pq.withReviewer.Clone(),
...
func (pq *PostQuery) WithReviewer(opts ...func(*UserQuery)) *PostQuery {
	query := (&UserClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withReviewer = query
	return pq
}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withReviewer; query != nil {
		if err := pq.loadReviewer(ctx, query, nodes, nil,
			func(n *Post, e *User) { n.Edges.Reviewer = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PostQuery) loadReviewer(ctx context.Context, query *UserQuery, nodes []*Post, init func(*Post), assign func(*Post, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Post)
	for i := range nodes {
...
func (pq *PostQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withReviewer != nil {
			_spec.Node.AddColumnOnce(post.FieldReviewerID)
		}
	}
