...
// - transport → draft_wiring.go → entapi runtime → ent.Client
...
// DraftCreateRequest is the create request for Draft.
//
...
// draftCreateRequestTags is the canonical JSON key of every field on
// DraftCreateRequest, in declaration order. UnmarshalJSON needs the tags as
// data, not as struct tags, to tell a case variant from an unrelated key.
var draftCreateRequestTags = []string{"headline"}

// DraftCreateRequestTags returns the canonical JSON key of every field on
// DraftCreateRequest, in declaration order. Generated handlers compare raw
...
func DraftCreateRequestTags() []string {
	return append([]string(nil), draftCreateRequestTags...)
}
...
func (r *DraftCreateRequest) UnmarshalJSON(b []byte) error {
	type alias DraftCreateRequest // breaks the UnmarshalJSON recursion
	var raw map[string]json.RawMessage
	if err := json.Unmarshal(b, &raw); err != nil {
		return err
	}
	for k := range raw {
		for _, t := range draftCreateRequestTags {
...
// everything.
func (r *DraftCreateRequest) has(field string) bool {
	if r.present == nil {
		return true
	}
...
// HasHeadline reports whether the payload carried "headline".
func (r *DraftCreateRequest) HasHeadline() bool { return r.has("headline") }

// ValidDraftCreateRequest wraps a DraftCreateRequest that has
...
type ValidDraftCreateRequest struct{ r *DraftCreateRequest }

// HasHeadline reports whether the payload carried "headline".
func (v *ValidDraftCreateRequest) HasHeadline() bool { return v.r.HasHeadline() }

// Validate checks the request and returns the only type Apply accepts.
func (r *DraftCreateRequest) Validate() (*ValidDraftCreateRequest, error) {
	if r == nil {
		return nil, fmt.Errorf("%w: create request is nil", entapi.ErrValidation)
	}
	if r.Headline == "" {
		return nil, fmt.Errorf("%w: headline is required", entapi.ErrValidation)
	}
	return &ValidDraftCreateRequest{r: r}, nil
}
...
func (v *ValidDraftCreateRequest) Apply(b *DraftCreate) *DraftCreate {
	r := v.r
	b.SetHeadline(r.Headline)
	return b
}

// DraftPatchRequest is the partial-update request for Draft.
//
...
// draftPatchRequestTags is the canonical JSON key of every field on
// DraftPatchRequest, in declaration order.
var draftPatchRequestTags = []string{"headline"}

// DraftPatchRequestTags returns the canonical JSON key of every field on
// DraftPatchRequest, in declaration order. Generated handlers compare raw
// request keys against this data to reject unknown keys.
//
// Like Routes(), it returns a fresh copy on every call: slices are mutable, so
// exporting the variable would let any consumer mutate validation data that
...
// the source of truth; this accessor is for consumers that decline the generated
// handler and write their own bind step.
func DraftPatchRequestTags() []string {
	return append([]string(nil), draftPatchRequestTags...)
}
...
func (r *DraftPatchRequest) UnmarshalJSON(b []byte) error {
	type alias DraftPatchRequest // breaks the UnmarshalJSON recursion
	var raw map[string]json.RawMessage
	if err := json.Unmarshal(b, &raw); err != nil {
		return err
	}
	for k := range raw {
		for _, t := range draftPatchRequestTags {
			if k != t && strings.EqualFold(k, t) {
				return fmt.Errorf("%w: unknown key %q (did you mean %q?)", entapi.ErrValidation, k, t)
			}
		}
...
// nothing for it.
func (r *DraftPatchRequest) has(field string) bool { return r.present[field] }

// HasHeadline reports whether the payload carried "headline".
func (r *DraftPatchRequest) HasHeadline() bool { return r.has("headline") }

// ValidDraftPatchRequest wraps a DraftPatchRequest that has passed
// Validate. It is the only type Apply is defined on.
type ValidDraftPatchRequest struct{ r *DraftPatchRequest }
...
func (v *ValidDraftPatchRequest) HasHeadline() bool { return v.r.HasHeadline() }

// Headline reports the value the payload carried for
// "headline", and whether Apply will Set it.
//
// Presence is two of the three states a PATCH field has; this reads the third.
// Paired with HasHeadline(), which is "present" below:
//
...
func (v *ValidDraftPatchRequest) Headline() (string, bool) {
	if !v.r.HasHeadline() || v.r.Headline == nil {
		var zero string
		return zero, false
	}
	return *v.r.Headline, true
}
...
func (r *DraftPatchRequest) Validate() (*ValidDraftPatchRequest, error) {
	if r == nil {
		return nil, fmt.Errorf("%w: patch request is nil", entapi.ErrValidation)
	}
	if r.HasHeadline() && r.Headline == nil {
		return nil, fmt.Errorf("%w: headline cannot be null", entapi.ErrValidation)
	}
	return &ValidDraftPatchRequest{r: r}, nil
}
...
func (v *ValidDraftPatchRequest) Apply(b *DraftUpdateOne) *DraftUpdateOne {
	r := v.r
	if r.HasHeadline() {
		b.SetHeadline(*r.Headline)
	}
...
// DraftSummary is the shape Draft takes on another entity's response.
//
// It carries the same scalar fields as DraftResponse and no edges at all.
// That is what bounds expansion: NewDraftResponse calls summary
...
// NewDraftSummary converts an entity to its summary DTO. It cannot fail:
// a summary reads no edges.
func NewDraftSummary(e *Draft) *DraftSummary {
	if e == nil {
		return nil
	}
	return &DraftSummary{
...
// DraftResponse represents the response for Draft.
//
...
// DraftListResponse below refers to this type either way.
//
...
// NewDraftResponse converts an entity to its response DTO.
//
...
// reads as "this draft has none".
//
// That error is cheap because DraftQueryWithResponseEdges below is
...
func NewDraftResponse(e *Draft) (*DraftResponse, error) {
	if e == nil {
		return nil, nil
	}
	r := &DraftResponse{
	...
	docEdge, err := e.Edges.DocOrErr()
	switch {
	case err == nil:
		r.Doc = NewDocSummary(docEdge)
	case IsNotFound(err):
...
// DraftQueryWithResponseEdges applies the eager-load plan for DraftResponse.
//
...
// NewDraftResponse return a not-loaded-edge error at run time. It fails
// loudly; it never silently narrows the response.
//
// DraftClient.Get loads no edges and therefore cannot serve this response
// type. Wiring for Draft must go through Query with this plan applied.
func DraftQueryWithResponseEdges(q *DraftQuery) *DraftQuery {
	return q.WithDoc()
}

// DraftListResponse is the NAMED, non-generic shape of one list page —
// for OpenAPI/swaggo-class annotation tooling, which cannot express the
// generic entapi.Page[DraftResponse] that ListDrafts returns.
// Convert at the handler boundary with NewDraftListResponse.
...
// NewDraftListResponse converts the page ListDrafts returns into
// the named list shape. The conversion expression is the shape contract:
// if DraftListResponse and entapi.Page ever diverge in field set,
...
func NewDraftListResponse(p *entapi.Page[DraftResponse]) *DraftListResponse {
	if p == nil {
		return nil
	}
	r := DraftListResponse(*p)
	return &r
}
