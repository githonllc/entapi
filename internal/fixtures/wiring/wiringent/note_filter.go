// Code generated by entapi extension from schema "Note". DO NOT EDIT.
// Source template: templates/filter.tmpl
// Regenerate with: make generate

package wiringent

import (
	"encoding"
	"fmt"
	"net/url"
	"sort"
	"strings"

	"entgo.io/ent/dialect/sql"
	"github.com/githonllc/entapi/internal/fixtures/wiring/wiringent/note"
	"github.com/githonllc/entapi/internal/fixtures/wiring/wiringent/predicate"
	entapi "github.com/githonllc/entapi/runtime"
	"github.com/google/uuid"
)

// NoteFilter is the typed query representation for Note.
// Repeated URL parameters occupy repeated slots and Predicates ANDs them.
type NoteFilter struct {
	// id: uuid.UUID
	ID      []uuid.UUID   `json:"id,omitempty"`
	IDNEQ   []uuid.UUID   `json:"id_neq,omitempty"`
	IDIn    [][]uuid.UUID `json:"id_in,omitempty"`
	IDNotIn [][]uuid.UUID `json:"id_not_in,omitempty"`
	IDGT    []uuid.UUID   `json:"id_gt,omitempty"`
	IDGTE   []uuid.UUID   `json:"id_gte,omitempty"`
	IDLT    []uuid.UUID   `json:"id_lt,omitempty"`
	IDLTE   []uuid.UUID   `json:"id_lte,omitempty"`
	// body: string
	Body          []string   `json:"body,omitempty"`
	BodyNEQ       []string   `json:"body_neq,omitempty"`
	BodyIn        [][]string `json:"body_in,omitempty"`
	BodyNotIn     [][]string `json:"body_not_in,omitempty"`
	BodyGT        []string   `json:"body_gt,omitempty"`
	BodyGTE       []string   `json:"body_gte,omitempty"`
	BodyLT        []string   `json:"body_lt,omitempty"`
	BodyLTE       []string   `json:"body_lte,omitempty"`
	BodyHasPrefix []string   `json:"body_prefix,omitempty"`
}

// Predicates converts the typed filter into conjunctive ent predicates.
func (f *NoteFilter) Predicates() []predicate.Note {
	if f == nil {
		return nil
	}
	var ps []predicate.Note
	entapi.AppendEach(&ps, f.ID, note.IDEQ)
	entapi.AppendEach(&ps, f.IDNEQ, note.IDNEQ)
	entapi.AppendEachSlice(&ps, f.IDIn, note.IDIn)
	entapi.AppendEachSlice(&ps, f.IDNotIn, note.IDNotIn)
	entapi.AppendEach(&ps, f.IDGT, note.IDGT)
	entapi.AppendEach(&ps, f.IDGTE, note.IDGTE)
	entapi.AppendEach(&ps, f.IDLT, note.IDLT)
	entapi.AppendEach(&ps, f.IDLTE, note.IDLTE)
	entapi.AppendEach(&ps, f.Body, note.BodyEQ)
	entapi.AppendEach(&ps, f.BodyNEQ, note.BodyNEQ)
	entapi.AppendEachSlice(&ps, f.BodyIn, note.BodyIn)
	entapi.AppendEachSlice(&ps, f.BodyNotIn, note.BodyNotIn)
	entapi.AppendEach(&ps, f.BodyGT, note.BodyGT)
	entapi.AppendEach(&ps, f.BodyGTE, note.BodyGTE)
	entapi.AppendEach(&ps, f.BodyLT, note.BodyLT)
	entapi.AppendEach(&ps, f.BodyLTE, note.BodyLTE)
	entapi.AppendEach(&ps, f.BodyHasPrefix, note.BodyHasPrefix)
	return ps
}

func parseNoteIDQueryValue(raw, whole string) (uuid.UUID, error) {
	var parsed uuid.UUID
	if err := encoding.TextUnmarshaler(&parsed).UnmarshalText([]byte(raw)); err != nil {
		return parsed, fmt.Errorf("%w: field %q value %q is invalid: %v", entapi.ErrValidation, "id", whole, err)
	}
	return parsed, nil
}

func parseNoteBodyQueryValue(raw, whole string) (string, error) {
	return string(raw), nil
}

// ParseNoteQuery parses the complete URL query contract in sorted-key
// order. Runtime code owns lexical grammar; this generated switch owns the
// field-local operator set, conversion and predicate slots.
func ParseNoteQuery(q url.Values) (*NoteFilter, entapi.ListRequest, error) {
	f := &NoteFilter{}
	var request entapi.ListRequest
	keys := make([]string, 0, len(q))
	for key := range q {
		keys = append(keys, key)
	}
	sort.Strings(keys)
	for _, key := range keys {
		values := q[key]
		reserved, recognized, err := entapi.ReservedQueryValue(key, values)
		if err != nil {
			return nil, entapi.ListRequest{}, err
		}
		if recognized {
			switch key {
			case "_sort":
				request.Sort, err = entapi.ParseSortSpecs(reserved)
			case "_page":
				request.Page, err = entapi.ParsePageParam(key, reserved)
			case "_size":
				request.Size, err = entapi.ParsePageParam(key, reserved)
			case "_q":
				err = fmt.Errorf("%w: _q value %q is invalid because Note has no Searchable fields", entapi.ErrValidation, reserved)
			}
			if err != nil {
				return nil, entapi.ListRequest{}, err
			}
			continue
		}
		if strings.HasPrefix(key, "_") {
			return nil, entapi.ListRequest{}, fmt.Errorf("%w: unknown reserved query parameter %q", entapi.ErrValidation, key)
		}
		switch key {
		case "id":
			for _, whole := range values {
				if whole == "" {
					continue
				}
				op, raw, explicit := entapi.SplitOp(whole)
				if !explicit {
					op = "eq"
				}
				switch op {
				case "eq":
				case "ne":
				case "in":
				case "not_in":
				case "gt":
				case "ge":
				case "lt":
				case "le":
				case "from":
				case "to":
				case "between":
				default:
					if entapi.KnownQueryOperator(op) {
						return nil, entapi.ListRequest{}, fmt.Errorf("%w: field %q value %q uses operator %q; legal operators: %s", entapi.ErrValidation, key, whole, op, "eq, ne, in, not_in, gt, ge, lt, le, from, to, between")
					}
					op, raw = "eq", whole
				}
				switch op {
				case "eq":
					parsed, parseErr := parseNoteIDQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.ID = append(f.ID, parsed)
				case "ne":
					parsed, parseErr := parseNoteIDQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.IDNEQ = append(f.IDNEQ, parsed)
				case "in":
					parts := strings.Split(raw, ",")
					parsedValues := make([]uuid.UUID, 0, len(parts))
					for _, part := range parts {
						parsed, parseErr := parseNoteIDQueryValue(part, whole)
						if parseErr != nil {
							return nil, entapi.ListRequest{}, parseErr
						}
						parsedValues = append(parsedValues, parsed)
					}
					f.IDIn = append(f.IDIn, parsedValues)
				case "not_in":
					parts := strings.Split(raw, ",")
					parsedValues := make([]uuid.UUID, 0, len(parts))
					for _, part := range parts {
						parsed, parseErr := parseNoteIDQueryValue(part, whole)
						if parseErr != nil {
							return nil, entapi.ListRequest{}, parseErr
						}
						parsedValues = append(parsedValues, parsed)
					}
					f.IDNotIn = append(f.IDNotIn, parsedValues)
				case "gt":
					parsed, parseErr := parseNoteIDQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.IDGT = append(f.IDGT, parsed)
				case "ge":
					parsed, parseErr := parseNoteIDQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.IDGTE = append(f.IDGTE, parsed)
				case "lt":
					parsed, parseErr := parseNoteIDQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.IDLT = append(f.IDLT, parsed)
				case "le":
					parsed, parseErr := parseNoteIDQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.IDLTE = append(f.IDLTE, parsed)
				case "from":
					parsed, parseErr := parseNoteIDQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.IDGTE = append(f.IDGTE, parsed)
				case "to":
					parsed, parseErr := parseNoteIDQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.IDLTE = append(f.IDLTE, parsed)
				case "between":
					parts := strings.Split(raw, ",")
					if len(parts) != 2 {
						return nil, entapi.ListRequest{}, fmt.Errorf("%w: field %q value %q uses between with %d parts; exactly two are required", entapi.ErrValidation, key, whole, len(parts))
					}
					lower, parseErr := parseNoteIDQueryValue(parts[0], whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					upper, parseErr := parseNoteIDQueryValue(parts[1], whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.IDGTE = append(f.IDGTE, lower)
					f.IDLTE = append(f.IDLTE, upper)
				}
			}
		case "body":
			for _, whole := range values {
				if whole == "" {
					continue
				}
				op, raw, explicit := entapi.SplitOp(whole)
				if !explicit {
					op = "eq"
				}
				switch op {
				case "eq":
				case "ne":
				case "in":
				case "not_in":
				case "gt":
				case "ge":
				case "lt":
				case "le":
				case "prefix":
				case "from":
				case "to":
				case "between":
				default:
					if entapi.KnownQueryOperator(op) {
						return nil, entapi.ListRequest{}, fmt.Errorf("%w: field %q value %q uses operator %q; legal operators: %s", entapi.ErrValidation, key, whole, op, "eq, ne, in, not_in, gt, ge, lt, le, prefix, from, to, between")
					}
					op, raw = "eq", whole
				}
				switch op {
				case "eq":
					parsed, parseErr := parseNoteBodyQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.Body = append(f.Body, parsed)
				case "ne":
					parsed, parseErr := parseNoteBodyQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.BodyNEQ = append(f.BodyNEQ, parsed)
				case "in":
					parts := strings.Split(raw, ",")
					parsedValues := make([]string, 0, len(parts))
					for _, part := range parts {
						parsed, parseErr := parseNoteBodyQueryValue(part, whole)
						if parseErr != nil {
							return nil, entapi.ListRequest{}, parseErr
						}
						parsedValues = append(parsedValues, parsed)
					}
					f.BodyIn = append(f.BodyIn, parsedValues)
				case "not_in":
					parts := strings.Split(raw, ",")
					parsedValues := make([]string, 0, len(parts))
					for _, part := range parts {
						parsed, parseErr := parseNoteBodyQueryValue(part, whole)
						if parseErr != nil {
							return nil, entapi.ListRequest{}, parseErr
						}
						parsedValues = append(parsedValues, parsed)
					}
					f.BodyNotIn = append(f.BodyNotIn, parsedValues)
				case "gt":
					parsed, parseErr := parseNoteBodyQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.BodyGT = append(f.BodyGT, parsed)
				case "ge":
					parsed, parseErr := parseNoteBodyQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.BodyGTE = append(f.BodyGTE, parsed)
				case "lt":
					parsed, parseErr := parseNoteBodyQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.BodyLT = append(f.BodyLT, parsed)
				case "le":
					parsed, parseErr := parseNoteBodyQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.BodyLTE = append(f.BodyLTE, parsed)
				case "prefix":
					parsed, parseErr := parseNoteBodyQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.BodyHasPrefix = append(f.BodyHasPrefix, parsed)
				case "from":
					parsed, parseErr := parseNoteBodyQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.BodyGTE = append(f.BodyGTE, parsed)
				case "to":
					parsed, parseErr := parseNoteBodyQueryValue(raw, whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.BodyLTE = append(f.BodyLTE, parsed)
				case "between":
					parts := strings.Split(raw, ",")
					if len(parts) != 2 {
						return nil, entapi.ListRequest{}, fmt.Errorf("%w: field %q value %q uses between with %d parts; exactly two are required", entapi.ErrValidation, key, whole, len(parts))
					}
					lower, parseErr := parseNoteBodyQueryValue(parts[0], whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					upper, parseErr := parseNoteBodyQueryValue(parts[1], whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					f.BodyGTE = append(f.BodyGTE, lower)
					f.BodyLTE = append(f.BodyLTE, upper)
				}
			}
		default:
			switch key {
			default:
				return nil, entapi.ListRequest{}, fmt.Errorf("%w: unknown query field %q with value %q", entapi.ErrValidation, key, values)
			}
		}
	}
	return f, request, nil
}

var noteSortOptions = map[string]func(...sql.OrderTermOption) note.OrderOption{
	"id":   note.ByID,
	"body": note.ByBody,
}

// NoteSortKeys is the complete sort allow-list. The primary key is
// naturally Sortable; every other key requires api.Sortable().
var NoteSortKeys = []string{
	"id", "body",
}

// NoteOrder is the single sort allow-list seam.
func NoteOrder(r entapi.ListRequest) ([]note.OrderOption, error) {
	if len(r.Sort) == 0 {
		return []note.OrderOption{note.ByID(sql.OrderAsc())}, nil
	}
	order := make([]note.OrderOption, 0, len(r.Sort)+1)
	idPresent := false
	lastDir := sql.OrderAsc()
	for _, spec := range r.Sort {
		by, ok := noteSortOptions[spec.Key]
		if !ok {
			return nil, fmt.Errorf("%w: cannot sort by %q; legal keys: %s", entapi.ErrValidation, spec.Key, strings.Join(NoteSortKeys, ", "))
		}
		dir := sql.OrderAsc()
		if spec.Desc {
			dir = sql.OrderDesc()
		}
		order = append(order, by(dir))
		lastDir = dir
		if spec.Key == "id" {
			idPresent = true
		}
	}
	if !idPresent {
		order = append(order, note.ByID(lastDir))
	}
	return order, nil
}
