...
func (alq *AuditLogQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("httpdemoent: check existence: %w", err)
	default:
		return true, nil
	}
}
...
//		GroupBy(auditlog.FieldMessage).
//		Aggregate(httpdemoent.Count()).
//		Scan(ctx, &v)
...
func (alq *AuditLogQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range alq.inters {
		if inter == nil {
			return fmt.Errorf("httpdemoent: uninitialized interceptor (forgotten import httpdemoent/runtime?)")
		}
	...
	for _, f := range alq.ctx.Fields {
		if !auditlog.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("httpdemoent: invalid field %q for query", f)}
		}
	}
	if alq.path != nil {
