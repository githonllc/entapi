...
var docCreateRequestTags = []string{"title"}

// DocCreateRequestTags returns the canonical JSON key of every field on
...
func DocCreateRequestTags() []string {
	return append([]string(nil), docCreateRequestTags...)
}
...
// DocPatchRequestTags returns the canonical JSON key of every field on
...
// the source of truth; this accessor is for consumers that decline the generated
// handler and write their own bind step.
func DocPatchRequestTags() []string {
	return append([]string(nil), docPatchRequestTags...)
}
...
func (v *ValidDocPatchRequest) Title() (string, bool) {
...
// reads as "this doc has none".
//
// That error is cheap because DocQueryWithResponseEdges below is
...
func NewDocResponse(e *Doc) (*DocResponse, error) {
	...
	noteEdge, err := e.Edges.NoteOrErr()
	switch {
	case err == nil:
		r.Note = NewNoteSummary(noteEdge)
	case IsNotFound(err):
...
// NewDocResponse return a not-loaded-edge error at run time. It fails
// loudly; it never silently narrows the response.
//
// DocClient.Get loads no edges and therefore cannot serve this response
// type. Wiring for Doc must go through Query with this plan applied.
