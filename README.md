...
> aliases.
>
> The code itself is complete. Both redesigns — `docs/DESIGN-v2.md` and
> `docs/DESIGN-v3-final.md` — have landed in full; their remaining deviations
> are listed in the guide under
> [Deviations from DESIGN-v2](docs/GUIDE.md#deviations-from-design-v2) and
> [Deviations from DESIGN-v3](docs/GUIDE.md#deviations-from-design-v3). Known
> defects are in [`docs/QUALITY-REVIEW.md`](docs/QUALITY-REVIEW.md).
>
...
- [Gotchas](#gotchas) · [Limits](#limits) · [Migration notes](#migration-notes)
...
[Migration from the scope model](docs/GUIDE.md#migration-from-the-scope-model)
...
line carries `Code generated by entapi extension` — or the pre-rename
`Code generated by entdomain extension`, recognised permanently rather than for
a transition window — and that this run did not write. **That marker line is
your escape hatch:** delete it and the file is yours.
...
The old scope-model annotation vocabulary (`DefaultField()`, `ScopeCreate`,
`AsSearchable`, `Edge().InResponse()`, …) maps to the current one in
[Migration from the scope model](docs/GUIDE.md#migration-from-the-scope-model).
