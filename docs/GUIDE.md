...
The complete reference. [`README.md`](../README.md) is the entry point — what
this extension is, how to run an example, and the tables you need most often.
This file is the long form of every section it summarises: what each rule is,
...
- [Generated HTTP](#generated-http) — customization points, the OpenAPI document, ogen, endpoint registration, hand-written handlers
...
The split is load-bearing, not cosmetic. The root package embeds ten templates
with `//go:embed` and reads all ten out of the embedded filesystem **at package
...
> `handlerTemplate`, `errorMapTemplate`, `httpTemplate`, `openapiTemplate`,
> `openapiEmbedTemplate`, `softDeleteTemplate`,
> `softDeleteConfigInitTemplate` (ten package-level `var`s, all
...
Two working copies of this file are in the repository, each beside a
`generate.go` holding the `//go:generate go run -mod=mod entc.go` line that
actually runs it: [`examples/todo/todoent/entc.go`](../examples/todo/todoent/entc.go)
is the minimal one, and
[`examples/petstore/petstoreent/entc.go`](../examples/petstore/petstoreent/entc.go)
additionally sets the OpenAPI title and version. Forgetting `generate.go` is the
common first mistake: `go generate ./...` then silently does nothing.
...
> **Implementation:** `api/annotations.go` — `ResourceAnnotation`,
> `FieldAnnotation`, `EdgeAnnotation` and their `Merge` methods;
> `funcs_scope.go` — `getResourceAnnotation`, `getFieldAnnotation`,
> `getEdgeAnnotation` (the three normalized readers) over
> `decodeResourceAnnotation` / `decodeAnnotation` (the JSON round trip), plus
> `isResource`, `resourceExcepts`, `hasCreateFamily` and `blockedCreateFields`
> — the last being what decides whether a create family is provably unusable;
...
> **Implementation:** `templates/http.tmpl` — `APIHandler`, `API`, `With`,
> `ServeHTTP`, `Mount`, `Endpoints`, the per-operation `…Endpoint()` accessors
> and `OpenAPIEndpoint()`; `templates/handler.tmpl` — the bind → call → write
> bodies and the `{Op}{Entity}Fn` types; `templates/openapi.tmpl` and
> `templates/openapi_embed.tmpl`; `funcs_http.go` — `resourceOperation`,
> `resourceOps`, `routePath`, `idParseExpr`, `handlerImports`, `entSnake` /
> `mustEntSnake`; `funcs_openapi.go` — `yamlQuote`, `openapiSchema`,
> `openapiFieldType`, `openapiPathGroup` / `openapiPathGroups`, `openapiParam`,
> `openapiFilterParams`, `openapiFilterDescription`, `openapiReservedParams`,
> `openapiSortKeys`, `openapiRequiredCreateFields`, `openapiStatus` /
> `newOpenAPIStatus`, `errorStatusesByOp`, `openapiErrorStatuses`,
> `openapiProblemStatuses`, `defaultOpenAPIVersion`; `runtime/http.go` —
> `Endpoint`, `Op` with `OpNone`/`OpList`/`OpCreate`/`OpGet`/`OpPatch`/`OpDelete`,
> `FieldError`, `problem`, `WriteProblem`, `WithActor`, `ActorFrom`;
> `runtime/endpoint.go` — `ColonPath`, `Endpoint.Bind`, `routePlaceholder`;
> `runtime/bind.go` — `BindJSON`, `Status`, `WriteJSON`, `classifiedError`,
> `classifyError`; generated examples:
> `internal/fixtures/httpdemo/httpdemoent/entapi_http.go` and its
> `openapi.yaml`; behavioural proof: `internal/fixtures/httpdemo/e2e`
...
[ADR-0001](adr/0001-presence-follows-encoding-json-key-matching.md).
...
### The split, and the primitives the runtime exports

The division of labour is the load-bearing part: **the runtime owns only lexical
grammar, generated code owns semantic permission.** The runtime cannot know
which operators a field allows — that is `$field.Ops` intersected with the wire
vocabulary, and it is decided at generation time — so it exports the lexical
half and nothing more:

| Runtime function | Answers |
|---|---|
| `SplitOp(value)` | split on the first colon: `(op, operand, explicit)`; no colon means `explicit == false` and the whole value is the operand |
| `KnownQueryOperator(op)` | is `op` in the global wire vocabulary at all — the seventeen prefixes in the table above. Field-specific permission is **not** checked here |
| `ReservedQueryValue(key, values)` | is `key` one of the four `_`-prefixed parameters, and is it single-valued |
| `ParsePageParam(name, value)` | the positive one-indexed `_page`/`_size` integers, with `_size=0` refused by name |
| `ParseSortSpecs(value)` | `_sort` into `[]SortSpec`; direction must be `asc` or `desc`. Key permission is **not** checked here — generated `{Entity}Order` is the single allow-list seam |

Two of those five are deliberately incomplete on their own, and that is the
design rather than an omission: `KnownQueryOperator` says only that a prefix
exists somewhere in the vocabulary, and `ParseSortSpecs` accepts any key. A
consumer writing a parser by hand gets the lexical layer for free and still has
to supply the allow-list — which is exactly what `Parse{Entity}Query` and
`{Entity}Order` are.
...
Everything field-specific arrives as an argument — `ops` is the field's allowed
operator rows, `parse` is its conversion function, `legal` is the pre-joined
operator list quoted verbatim in error messages, and `strict` is
`WithStrictQueryOperators()` frozen at generation time. What the runtime keeps is
only what every field shares: the operator split, the `eq` fallback, comma
splitting, and wrapping every failure as `ErrValidation`.

A `QueryOp[T]` row carries a `Prefix`, an `OpKind`, and **typed pointers into
the generated filter struct** — `One`, `Second`, `Many`, `Null` — so dispatch
uses no reflection and no string-keyed lookup of the predicate target, and a
missing or wrong-typed filter field is a compile error in the generated package.
`OpKind` has exactly four shapes:

| `OpKind` | Operand | Appends to |
|---|---|---|
| `OpKindValue` | one value | `One` |
| `OpKindList` | comma-separated, each parsed | `Many`, as one entry |
| `OpKindFlag` | must be **empty** (`is_null:` / `not_null:`) | `Null`, the row's `BoolValue` |
| `OpKindRange` | exactly two comma-separated parts (`between:`) | lower to `One`, upper to `Second` |

One honest residue, stated in the type's own comment: the pairing between `Kind`
and which slot pointer is set is a contract `filter.tmpl` upholds as the sole
construction site, **not** something the compiler checks. `ParseFieldValues`
catches only a `Kind` outside the four — reported as a plain error rather than a
validation failure — while a row whose `Kind` is valid but whose matching slot is
nil panics on the nil dereference.

> **Implementation:** `funcs_filter.go` — `queryFields`, `parseFields`,
> `searchFields`, `unfilterableFields`, `isSortable`, `fieldParseOperators`,
> `fieldParseConversion`, `opWirePrefix`, `substringOps`, `filterImports`;
> `runtime/types.go` — `ListRequest`, `SortSpec`, `DefaultPageSize` (20),
> `MaxPageSize` (1000); `runtime/urlquery.go` — `queryOperators` (the
> seventeen-entry vocabulary), `reservedQueryParams` (the four), `SplitOp`,
...
Four graph-level names are reserved: `ErrorMap`, and `API`, `APIHandler` and
`APIOption` together — the last three because `entapi_http.go` declares all of
them, so an entity named after any one of the three collides exactly as hard as
one named after the handler type. Each is gated on the condition its own file is
written under, which is why they are checked here and not in the node loop. An
entity called `ErrorMap` makes ent emit `type ErrorMap` while
...
> **Implementation:** `schema_conflicts.go` — `checkGraphConflicts` (the graph
> entry point), `nodeConflicts` (the per-node dispatcher, which also carries the
> empty-PATCH check inline rather than in a named function of its own),
> `fieldWordConflicts`, `queryConflicts`, `misplacedExpandConflict`,
> `requiredEdgeWithoutFieldConflicts`, `requiredEdgeWithoutFieldConflict`,
...
> `patchPresenceCollision`, `fieldHasOp`, `hasRawAnnotation`, `markerList`,
> `dimensionList`, `errorMapSymbol`, `apiSymbol`, `apiHandlerSymbol`,
> `apiOptionSymbol`, `mustEntPlural`
...
platforms.
([ADR-0003](adr/0003-per-run-atomic-generation.md))
...
> ([ADR-0004](adr/0004-cleanup-ownership-by-marker.md))

> **Implementation:** `extension.go` — `generatePerTypeFiles` (the two-phase
> loop), `formatFile`, `writeFormatted`, `pendingFile`; `cleanup.go` —
> `generatedMarker`, `legacyGeneratedMarker`, `markerScanBytes`,
> `removeStaleArtifacts`, `isCleanupCandidate`, `removeIfStale`,
> `hasGeneratedMarker`, and the five graph-level file-name constants
> (`softDeleteFileName`, `errorMapFileName`, `httpFileName`, `openapiFileName`,
> `openapiEmbedFileName`)
...
The silent ones, ordered by how quietly they hurt you, are in
[`README.md`](../README.md#gotchas). They live there rather than here because
they are the one list a reader has to see before adopting, not after.
