...
// - transport → article_wiring.go → entapi runtime → ent.Client
...
var articleCreateRequestTags = []string{"title", "rank", "author_id"}

// ArticleCreateRequestTags returns the canonical JSON key of every field on
// ArticleCreateRequest, in declaration order. Generated handlers compare raw
...
func ArticleCreateRequestTags() []string {
	return append([]string(nil), articleCreateRequestTags...)
}
...
func (r *ArticleCreateRequest) UnmarshalJSON(b []byte) error {
	...
	for k := range raw {
		for _, t := range articleCreateRequestTags {
...
var articlePatchRequestTags = []string{"title", "rank", "author_id"}

// ArticlePatchRequestTags returns the canonical JSON key of every field on
// ArticlePatchRequest, in declaration order. Generated handlers compare raw
// request keys against this data to reject unknown keys.
//
// Like Routes(), it returns a fresh copy on every call: slices are mutable, so
// exporting the variable would let any consumer mutate validation data that
...
// the source of truth; this accessor is for consumers that decline the generated
// handler and write their own bind step.
func ArticlePatchRequestTags() []string {
	return append([]string(nil), articlePatchRequestTags...)
}
...
func (r *ArticlePatchRequest) UnmarshalJSON(b []byte) error {
	...
	for k := range raw {
		for _, t := range articlePatchRequestTags {
			if k != t && strings.EqualFold(k, t) {
				return fmt.Errorf("%w: unknown key %q (did you mean %q?)", entapi.ErrValidation, k, t)
			}
		}
...
// the customization point is forced to allocate an update builder it never
// executes, Apply the request to it and read back Mutation() (#113).
func (v *ValidArticlePatchRequest) AuthorID() (uuid.UUID, bool) {
...
// ArticleListResponse is the NAMED, non-generic shape of one list page —
// for OpenAPI/swaggo-class annotation tooling, which cannot express the
// generic entapi.Page[ArticleResponse] that ListArticles returns.
// Convert at the handler boundary with NewArticleListResponse.
...
// NewArticleListResponse converts the page ListArticles returns into
// the named list shape. The conversion expression is the shape contract:
// if ArticleListResponse and entapi.Page ever diverge in field set,
...
func NewArticleListResponse(p *entapi.Page[ArticleResponse]) *ArticleListResponse {
	if p == nil {
		return nil
	}
	r := ArticleListResponse(*p)
	return &r
}
