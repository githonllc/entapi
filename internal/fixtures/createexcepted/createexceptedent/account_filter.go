...
// ============================================================================
// Query surface for Account — filters, free-text search and the sort
...
// AccountFilter is the structured query surface for Account.
//
...
// accountSortOptions maps an allowed sort key to ent's own order
// builder for that column. It is unexported: the mapping is reached through
// AccountOrder, which checks the key first.
var accountSortOptions = map[string]func(...sql.OrderTermOption) account.OrderOption{}

// AccountSortKeys is the sort allow-list: exactly the fields annotated
...
// AccountOrder turns a list request into ent order options.
//
// The caller's string is checked against AccountSortKeys and then thrown
...
func AccountOrder(r entapi.ListRequest) ([]account.OrderOption, error) {
	key, desc, err := r.SortKey(AccountSortKeys, "")
	if err != nil {
		return nil, err
	}
	dir := sql.OrderAsc()
	...
	by, ok := accountSortOptions[key]
	if !ok {
		return []account.OrderOption{account.ByID(sql.OrderAsc())}, nil
	}
	order := []account.OrderOption{by(dir)}
	if key != "id" {
		order = append(order, account.ByID(dir))
	}
	return order, nil
}
