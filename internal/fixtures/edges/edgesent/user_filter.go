...
// ============================================================================
// Query surface for User — filters, free-text search and the sort
...
// UserFilter is the structured query surface for User.
//
...
// userSortOptions maps an allowed sort key to ent's own order
// builder for that column. It is unexported: the mapping is reached through
// UserOrder, which checks the key first.
var userSortOptions = map[string]func(...sql.OrderTermOption) user.OrderOption{}

// UserSortKeys is the sort allow-list: exactly the fields annotated
...
// UserOrder turns a list request into ent order options.
//
// The caller's string is checked against UserSortKeys and then thrown
...
func UserOrder(r entapi.ListRequest) ([]user.OrderOption, error) {
	key, desc, err := r.SortKey(UserSortKeys, "")
	if err != nil {
		return nil, err
	}
	dir := sql.OrderAsc()
	...
	by, ok := userSortOptions[key]
	if !ok {
		return []user.OrderOption{user.ByID(sql.OrderAsc())}, nil
	}
	order := []user.OrderOption{by(dir)}
	if key != "id" {
		order = append(order, user.ByID(dir))
	}
	return order, nil
}
