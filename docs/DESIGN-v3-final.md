# entdomain v3 定稿：沉默即默认的注解模型 + 生成的 HTTP 层
...
注解住 **`entdomain/api`** 子包（纯 schema-time，不含任何运行时符号）。
...
2. 机械运行时助手（problem+json 写出、状态码映射、解析器共享部分）进
   **`entdomain/runtime`**——`net/http` 是 stdlib，stdlib-only 不变量不破;
...
- 生成到消费者 **`ent/openapi.yaml`**，随生成物 commit（PR diff 即暴露面
  审查）；首行自带 `# Code generated by entdomain extension` 注释 marker，
  cleanup 按该文件自己的 marker 决定可删性（ADR-0004 一致，删 marker 即接管）;
- 生成 `entdomain_openapi.go`：`go:embed` 该 yaml 并注册 `GET /openapi.yaml`
  ——磁盘与服务内容同源;
- 版本 **OpenAPI 3.1**。
...
func (User) Mixin() []ent.Mixin { return []ent.Mixin{entdomain.SoftDeleteMixin{}} }
