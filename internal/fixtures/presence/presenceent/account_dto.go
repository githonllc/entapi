...
func (r *AccountCreateRequest) Validate() (*ValidAccountCreateRequest, error) {
	...
	if !r.HasSeats() {
		return nil, fmt.Errorf("%w: seats is required", entapi.ErrValidation)
	}
	if !r.HasAcceptedTerms() {
		return nil, fmt.Errorf("%w: accepted_terms is required", entapi.ErrValidation)
	}
	if r.Origin == "" {
...
func (r *AccountPatchRequest) Validate() (*ValidAccountPatchRequest, error) {
	...
	if r.HasEmail() && r.Email == nil {
		return nil, fmt.Errorf("%w: email cannot be null", entapi.ErrValidation)
	}
	if r.HasSeats() && r.Seats == nil {
		return nil, fmt.Errorf("%w: seats cannot be null", entapi.ErrValidation)
	}
	if r.HasAcceptedTerms() && r.AcceptedTerms == nil {
		return nil, fmt.Errorf("%w: accepted_terms cannot be null", entapi.ErrValidation)
	}
	if r.HasPlan() && r.Plan == nil {
		return nil, fmt.Errorf("%w: plan cannot be null", entapi.ErrValidation)
	}
	if r.Plan != nil {
		if err := account.PlanValidator(*r.Plan); err != nil {
			return nil, fmt.Errorf("%w: %v", entapi.ErrValidation, err)
