...
func (au *ArticleUpdate) check() error {
	if au.mutation.AuthorCleared() && len(au.mutation.AuthorIDs()) > 0 {
		return errors.New(`wiringent: clearing a required unique edge "Article.author"`)
	}
	return nil
}
...
func (auo *ArticleUpdateOne) sqlSave(ctx context.Context) (_node *Article, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`wiringent: missing "Article.id" for update`)}
	}
