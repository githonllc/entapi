...
func (sq *SessionQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("sensitiveent: check existence: %w", err)
	default:
		return true, nil
	}
}
...
//		GroupBy(session.FieldUserAgent).
//		Aggregate(sensitiveent.Count()).
//		Scan(ctx, &v)
...
func (sq *SessionQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range sq.inters {
		if inter == nil {
			return fmt.Errorf("sensitiveent: uninitialized interceptor (forgotten import sensitiveent/runtime?)")
		}
	...
	for _, f := range sq.ctx.Fields {
		if !session.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("sensitiveent: invalid field %q for query", f)}
		}
	}
	if sq.path != nil {
...
func (sq *SessionQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*Session, init func(*Session), assign func(*Session, *Account)) error {
	...
	for i := range nodes {
		if nodes[i].session_account == nil {
			continue
		}
		fk := *nodes[i].session_account
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "session_account" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
