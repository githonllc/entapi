...
func (v *ValidUserPatchRequest) Apply(b *ent.UserUpdateOne) *ent.UserUpdateOne {
	r := v.r
	if r.HasName() {
		b.SetName(*r.Name)
	}
...
func ListUsers(ctx context.Context, db *ent.Client, f *UserFilter, r entdomain.ListRequest) (*entdomain.Page[UserResponse], error) {
	os, err := UserOrder(r)
	if err != nil {
		return nil, err
	}
