...
// ============================================================================
// Query surface for Ledger — filters, free-text search and the sort
...
// LedgerFilter is the structured query surface for Ledger.
//
...
// ledgerSortOptions maps an allowed sort key to ent's own order
// builder for that column. It is unexported: the mapping is reached through
// LedgerOrder, which checks the key first.
var ledgerSortOptions = map[string]func(...sql.OrderTermOption) ledger.OrderOption{}

// LedgerSortKeys is the sort allow-list: exactly the fields annotated
...
// LedgerOrder turns a list request into ent order options.
//
// The caller's string is checked against LedgerSortKeys and then thrown
...
func LedgerOrder(r entdomain.ListRequest) ([]ledger.OrderOption, error) {
	key, desc, err := r.SortKey(LedgerSortKeys, "")
	if err != nil {
		return nil, err
	}
	by, ok := ledgerSortOptions[key]
	if !ok {
		// Only reachable when no sort was requested and no default applies.
		return nil, nil
	}
	if desc {
		return []ledger.OrderOption{by(sql.OrderDesc())}, nil
	}
	return []ledger.OrderOption{by(sql.OrderAsc())}, nil
}
