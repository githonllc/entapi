...
func (pc *PostCreate) check() error {
	if _, ok := pc.mutation.Title(); !ok {
		return &ValidationError{Name: "title", err: errors.New(`spikeent: missing required field "Post.title"`)}
	}
	if _, ok := pc.mutation.AuthorID(); !ok {
		return &ValidationError{Name: "author_id", err: errors.New(`spikeent: missing required field "Post.author_id"`)}
	}
	if len(pc.mutation.AuthorIDs()) == 0 {
		return &ValidationError{Name: "author", err: errors.New(`spikeent: missing required edge "Post.author"`)}
	}
	return nil
}
