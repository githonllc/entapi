...
func (dq *DocQuery) QueryNote() *NoteQuery {
	query := (&NoteClient{config: dq.config}).Query()
...
func (dq *DocQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (dq *DocQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (dq *DocQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (dq *DocQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (dq *DocQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (dq *DocQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := dq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (dq *DocQuery) WithNote(opts ...func(*NoteQuery)) *DocQuery {
	query := (&NoteClient{config: dq.config}).Query()
...
func (dq *DocQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Doc, error) {
	...
	if query := dq.withNote; query != nil {
		if err := dq.loadNote(ctx, query, nodes, nil,
			func(n *Doc, e *Note) { n.Edges.Note = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DocQuery) loadNote(ctx context.Context, query *NoteQuery, nodes []*Doc, init func(*Doc), assign func(*Doc, *Note)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Doc)
	for i := range nodes {
		if nodes[i].note_docs == nil {
			continue
		}
		fk := *nodes[i].note_docs
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "note_docs" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (dq *DocQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(doc.Table, doc.Columns, sqlgraph.NewFieldSpec(doc.FieldID, field.TypeUUID))
	_spec.From = dq.sql
