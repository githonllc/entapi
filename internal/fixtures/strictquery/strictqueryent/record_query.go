...
func (rq *RecordQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("strictqueryent: check existence: %w", err)
	default:
		return true, nil
	}
}
...
//
//	client.Record.Query().
//		GroupBy(record.FieldKey).
//		Aggregate(strictqueryent.Count()).
//		Scan(ctx, &v)
...
func (rq *RecordQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range rq.inters {
		if inter == nil {
			return fmt.Errorf("strictqueryent: uninitialized interceptor (forgotten import strictqueryent/runtime?)")
		}
	...
	for _, f := range rq.ctx.Fields {
		if !record.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("strictqueryent: invalid field %q for query", f)}
		}
	}
	if rq.path != nil {
