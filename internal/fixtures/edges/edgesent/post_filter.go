...
// PostFilter is the typed query representation for Post.
...
func (f *PostFilter) Predicates() []predicate.Post {
	...
	entapi.AppendEach(&ps, f.ID, post.IDEQ)
	entapi.AppendEach(&ps, f.IDNEQ, post.IDNEQ)
	entapi.AppendEachSlice(&ps, f.IDIn, post.IDIn)
	entapi.AppendEachSlice(&ps, f.IDNotIn, post.IDNotIn)
	entapi.AppendEach(&ps, f.IDGT, post.IDGT)
	entapi.AppendEach(&ps, f.IDGTE, post.IDGTE)
	entapi.AppendEach(&ps, f.IDLT, post.IDLT)
	entapi.AppendEach(&ps, f.IDLTE, post.IDLTE)
	return ps
}

func parsePostIDQueryValue(raw, whole string) (uuid.UUID, error) {
...
// ParsePostQuery parses the complete URL query contract in sorted-key
...
func ParsePostQuery(q url.Values) (*PostFilter, entapi.ListRequest, error) {
	f := &PostFilter{}
	...
	for _, key := range keys {
		...
		if recognized {
			...
			case "_q":
				err = fmt.Errorf("%w: _q value %q is invalid because Post has no Searchable fields", entapi.ErrValidation, reserved)
			}
			if err != nil {
				return nil, entapi.ListRequest{}, err
			}
		...
		case "id":
			for _, whole := range values {
				...
				switch op {
				case "eq":
					parsed, parseErr := parsePostIDQueryValue(raw, whole)
					if parseErr != nil {
				...
				case "in":
					...
					for _, part := range parts {
						parsed, parseErr := parsePostIDQueryValue(part, whole)
						if parseErr != nil {
							return nil, entapi.ListRequest{}, parseErr
						}
				...
				case "between":
					...
					lower, parseErr := parsePostIDQueryValue(parts[0], whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					upper, parseErr := parsePostIDQueryValue(parts[1], whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
...
// PostSortKeys is the complete sort allow-list. The primary key is
// naturally Sortable; every other key requires api.Sortable().
var PostSortKeys = []string{
	"id",
}

// PostOrder is the single sort allow-list seam.
func PostOrder(r entapi.ListRequest) ([]post.OrderOption, error) {
	if len(r.Sort) == 0 {
	...
	for _, spec := range r.Sort {
		by, ok := postSortOptions[spec.Key]
		if !ok {
			return nil, fmt.Errorf("%w: cannot sort by %q; legal keys: %s", entapi.ErrValidation, spec.Key, strings.Join(PostSortKeys, ", "))
		}
		dir := sql.OrderAsc()
		if spec.Desc {
			dir = sql.OrderDesc()
