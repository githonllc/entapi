...
func (tc *TagCreate) AddPetIDs(ids ...int) *TagCreate {
	tc.mutation.AddPetIDs(ids...)
	return tc
}

// AddPets adds the "pets" edges to the Pet entity.
func (tc *TagCreate) AddPets(p ...*Pet) *TagCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return tc.AddPetIDs(ids...)
}
...
func (tc *TagCreate) check() error {
	if _, ok := tc.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`petstoreent: missing required field "Tag.name"`)}
	}
	return nil
}
...
func (tc *TagCreate) createSpec() (*Tag, *sqlgraph.CreateSpec) {
	...
	if nodes := tc.mutation.PetsIDs(); len(nodes) > 0 {
