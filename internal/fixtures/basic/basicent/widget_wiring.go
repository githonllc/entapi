...
func widgetGet(ctx context.Context, db *Client, id uuid.UUID) (*WidgetResponse, error) {
	return entapi.GetOne(ctx, widgetByID(db), NewWidgetResponse, id)
}
...
func ListWidgets(ctx context.Context, db *Client, f *WidgetFilter, r entapi.ListRequest) (*entapi.Page[WidgetResponse], error) {
	orderBy, err := WidgetOrder(r)
	if err != nil {
		return nil, err
	}
	p, err := entapi.ListPage(ctx, WidgetQueryWithResponseEdges(db.Widget.Query()), f.Predicates(), orderBy, r, NewWidgetResponse)
...
func CreateWidget(ctx context.Context, db *Client, v *ValidWidgetCreateRequest) (*WidgetResponse, error) {
	r, err := entapi.SaveOne(ctx, v.Apply(db.Widget.Create()), NewWidgetResponse)
	return r, ErrorMap.MapError(err)
}
