...
this extension's to overwrite or delete.

**That marker is the whole ownership rule, so know its blast radius.** After
writing this run's output, a generation run scans the target directory —
**top-level only**, never the subpackages ent generates below it — and deletes
every `.go` file whose first line carries the marker and which it did not just
write. That is what makes deleting an entity from your schema converge in one
run instead of leaving a `{entity}_dto.go` referencing builders ent no longer
generates. It also reaches the two files no template writes any more
(`{entity}_base_service.go`, `{entity}_base_handler.go`), which is how upgrading
past [#29](https://github.com/githonllc/entdomain/issues/29) cleans up after
itself. Files without the marker are never candidates, and each one cleanup
passes over is logged.

**To keep a generated file as your own: delete its first line.** An unmarked
file is not this extension's, so it is never overwritten and never deleted —
you own it from that point on, including the job of keeping it compiling.

**`{entity}_dto.go`'s header changed
