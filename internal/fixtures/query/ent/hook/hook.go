...
import (
	...
	"github.com/githonllc/entdomain/internal/fixtures/query/ent"
)

// The PlainFunc type is an adapter to allow the use of ordinary
// function as Plain mutator.
type PlainFunc func(context.Context, *ent.PlainMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PlainFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PlainMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PlainMutation", m)
}
