...
func (ac *ArticleCreate) SetRank(i int) *ArticleCreate {
	ac.mutation.SetRank(i)
	return ac
}

// SetNillableRank sets the "rank" field if the given value is not nil.
func (ac *ArticleCreate) SetNillableRank(i *int) *ArticleCreate {
	if i != nil {
		ac.SetRank(*i)
	}
	return ac
...
func (ac *ArticleCreate) check() error {
	if _, ok := ac.mutation.Title(); !ok {
		return &ValidationError{Name: "title", err: errors.New(`wiringent: missing required field "Article.title"`)}
	}
	if _, ok := ac.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`wiringent: missing required field "Article.created_at"`)}
	}
	if _, ok := ac.mutation.AuthorID(); !ok {
		return &ValidationError{Name: "author_id", err: errors.New(`wiringent: missing required field "Article.author_id"`)}
	}
	if len(ac.mutation.AuthorIDs()) == 0 {
		return &ValidationError{Name: "author", err: errors.New(`wiringent: missing required edge "Article.author"`)}
	}
	return nil
}
