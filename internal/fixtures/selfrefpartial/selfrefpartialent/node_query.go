...
func (nq *NodeQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("selfrefpartialent: check existence: %w", err)
	default:
		return true, nil
	}
}
...
//
//	client.Node.Query().
//		GroupBy(node.FieldLabel).
//		Aggregate(selfrefpartialent.Count()).
//		Scan(ctx, &v)
...
func (nq *NodeQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range nq.inters {
		if inter == nil {
			return fmt.Errorf("selfrefpartialent: uninitialized interceptor (forgotten import selfrefpartialent/runtime?)")
		}
	...
	for _, f := range nq.ctx.Fields {
		if !node.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("selfrefpartialent: invalid field %q for query", f)}
		}
	}
	if nq.path != nil {
