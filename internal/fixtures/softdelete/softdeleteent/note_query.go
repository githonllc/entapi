...
// QueryDocs chains the current query on the "docs" edge.
func (nq *NoteQuery) QueryDocs() *DocQuery {
	query := (&DocClient{config: nq.config}).Query()
...
// WithDocs tells the query-builder to eager-load the nodes that are connected to
// the "docs" edge. The optional arguments are used to configure the query builder of the edge.
func (nq *NoteQuery) WithDocs(opts ...func(*DocQuery)) *NoteQuery {
	query := (&DocClient{config: nq.config}).Query()
...
func (nq *NoteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Note, error) {
	...
	if query := nq.withDocs; query != nil {
		if err := nq.loadDocs(ctx, query, nodes,
			func(n *Note) { n.Edges.Docs = []*Doc{} },
			func(n *Note, e *Doc) { n.Edges.Docs = append(n.Edges.Docs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NoteQuery) loadDocs(ctx context.Context, query *DocQuery, nodes []*Note, init func(*Note), assign func(*Note, *Doc)) error {
	...
	query.Where(predicate.Doc(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(note.DocsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.note_docs
		if fk == nil {
			return fmt.Errorf(`foreign-key "note_docs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "note_docs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
