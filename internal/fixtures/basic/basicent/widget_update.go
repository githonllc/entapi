...
func (wuo *WidgetUpdateOne) sqlSave(ctx context.Context) (_node *Widget, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`basicent: missing "Widget.id" for update`)}
	}
	...
	if fields := wuo.fields; len(fields) > 0 {
		...
		for _, f := range fields {
			if !widget.ValidColumn(f) {
				return nil, &ValidationError{Name: f, err: fmt.Errorf("basicent: invalid field %q for query", f)}
			}
			if f != widget.FieldID {
