...
// - transport → author_wiring.go → entapi runtime → ent.Client
...
// AuthorCreateRequest is the create request for Author.
//
...
// AuthorCreateRequestTags returns the canonical JSON key of every field on
// AuthorCreateRequest, in declaration order. Generated handlers compare raw
...
func AuthorCreateRequestTags() []string {
	return append([]string(nil), authorCreateRequestTags...)
}
...
func (r *AuthorCreateRequest) UnmarshalJSON(b []byte) error {
	type alias AuthorCreateRequest // breaks the UnmarshalJSON recursion
	var raw map[string]json.RawMessage
	if err := json.Unmarshal(b, &raw); err != nil {
		return err
	}
	for k := range raw {
		for _, t := range authorCreateRequestTags {
...
// everything.
func (r *AuthorCreateRequest) has(field string) bool {
	if r.present == nil {
		return true
	}
...
func (r *AuthorCreateRequest) Validate() (*ValidAuthorCreateRequest, error) {
	...
	return &ValidAuthorCreateRequest{r: r}, nil
}
...
// AuthorPatchRequest is the partial-update request for Author.
//
...
// AuthorPatchRequestTags returns the canonical JSON key of every field on
// AuthorPatchRequest, in declaration order. Generated handlers compare raw
// request keys against this data to reject unknown keys.
//
// Like Routes(), it returns a fresh copy on every call: slices are mutable, so
// exporting the variable would let any consumer mutate validation data that
...
// the source of truth; this accessor is for consumers that decline the generated
// handler and write their own bind step.
func AuthorPatchRequestTags() []string {
	return append([]string(nil), authorPatchRequestTags...)
}
...
func (r *AuthorPatchRequest) UnmarshalJSON(b []byte) error {
	type alias AuthorPatchRequest // breaks the UnmarshalJSON recursion
	var raw map[string]json.RawMessage
	if err := json.Unmarshal(b, &raw); err != nil {
		return err
	}
	for k := range raw {
		for _, t := range authorPatchRequestTags {
			if k != t && strings.EqualFold(k, t) {
				return fmt.Errorf("%w: unknown key %q (did you mean %q?)", entapi.ErrValidation, k, t)
			}
		}
...
// nothing for it.
func (r *AuthorPatchRequest) has(field string) bool { return r.present[field] }
...
func (r *AuthorPatchRequest) Validate() (*ValidAuthorPatchRequest, error) {
	if r == nil {
	...
	return &ValidAuthorPatchRequest{r: r}, nil
}
...
func (v *ValidAuthorPatchRequest) Apply(b *AuthorUpdateOne) *AuthorUpdateOne {
	r := v.r
	if r.HasName() {
		b.SetName(*r.Name)
	}
...
// AuthorSummary is the shape Author takes on another entity's response.
//
// It carries the same scalar fields as AuthorResponse and no edges at all.
// That is what bounds expansion: NewAuthorResponse calls summary
...
func NewAuthorSummary(e *Author) *AuthorSummary {
	if e == nil {
		return nil
	}
...
// AuthorResponse represents the response for Author.
//
...
// AuthorListResponse below refers to this type either way.
//
...
// NewAuthorResponse converts an entity to its response DTO.
//
...
// reads as "this author has none".
//
// That error is cheap because AuthorQueryWithResponseEdges below is
...
func NewAuthorResponse(e *Author) (*AuthorResponse, error) {
	if e == nil {
		return nil, nil
	}
	r := &AuthorResponse{
	...
	articlesEdge, err := e.Edges.ArticlesOrErr()
	if err != nil {
		return nil, err
	}
	r.Articles = make([]*ArticleSummary, 0, len(articlesEdge))
	for _, v := range articlesEdge {
		r.Articles = append(r.Articles, NewArticleSummary(v))
	}
	return r, nil
}

// AuthorQueryWithResponseEdges applies the eager-load plan for AuthorResponse.
//
...
// NewAuthorResponse return a not-loaded-edge error at run time. It fails
// loudly; it never silently narrows the response.
//
// AuthorClient.Get loads no edges and therefore cannot serve this response
// type. Wiring for Author must go through Query with this plan applied.
func AuthorQueryWithResponseEdges(q *AuthorQuery) *AuthorQuery {
...
// generic entapi.Page[AuthorResponse] that ListAuthors returns.
...
// if AuthorListResponse and entapi.Page ever diverge in field set,
...
func NewAuthorListResponse(p *entapi.Page[AuthorResponse]) *AuthorListResponse {
	if p == nil {
		return nil
	}
