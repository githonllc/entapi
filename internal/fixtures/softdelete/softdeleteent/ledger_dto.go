...
// - transport → ledger_wiring.go → entdomain runtime → ent.Client
...
func (r *LedgerCreateRequest) Validate() (*ValidLedgerCreateRequest, error) {
	...
	if r.Entry == "" {
		return nil, fmt.Errorf("%w: entry is required", entdomain.ErrValidation)
	}
...
func (r *LedgerPatchRequest) Validate() (*ValidLedgerPatchRequest, error) {
	...
	if r.HasEntry() && r.Entry == nil {
		return nil, fmt.Errorf("%w: entry cannot be null", entdomain.ErrValidation)
	}
