...
// everything under internal/fixtures/<dir>/<dir>ent/ is generated. It is not
// regenerated and carries no DO NOT EDIT header.
//
// It has to live in the generated package (`edgesent`) because the contract it
// pins is precisely
...
func TestLoadedEdgePopulatesSummary(t *testing.T) {
	...
	// Compared as a type rather than against a spelled-out "*ent.UserSummary":
	// reflect renders the package name, and this package is named after its
	// directory (#49), so a literal would pin the fixture layout instead of the
	// contract. The contract is the summary TIER — *UserSummary, never *User.
	if got, want := reflect.TypeOf(r.Author), reflect.TypeOf(&UserSummary{}); got != want {
		t.Errorf("an edge must carry the summary tier, got %s, want %s", got, want)
	}
}
...
func TestToManyEdgeReachesTheResponse(t *testing.T) {
	...
	if want := reflect.TypeOf([]*PostSummary(nil)); f.Type != want {
		t.Errorf("Posts is %s, want %s", f.Type, want)
	}
