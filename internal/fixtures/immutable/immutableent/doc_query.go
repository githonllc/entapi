...
func (dq *DocQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("immutableent: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (dq *DocQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range dq.inters {
		if inter == nil {
			return fmt.Errorf("immutableent: uninitialized interceptor (forgotten import immutableent/runtime?)")
		}
	...
	for _, f := range dq.ctx.Fields {
		if !doc.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("immutableent: invalid field %q for query", f)}
		}
	}
	if dq.path != nil {
