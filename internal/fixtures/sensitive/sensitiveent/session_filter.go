	...
	for _, key := range keys {
		...
		case "id":
			for _, whole := range values {
				...
				switch op {
				case "eq":
					parsed, parseErr := parseSessionIDQueryValue(raw, whole)
					if parseErr != nil {
				...
				case "in":
					...
					for _, part := range parts {
						parsed, parseErr := parseSessionIDQueryValue(part, whole)
						if parseErr != nil {
							return nil, entapi.ListRequest{}, parseErr
						}
				...
				case "between":
					...
					lower, parseErr := parseSessionIDQueryValue(parts[0], whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					upper, parseErr := parseSessionIDQueryValue(parts[1], whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
