...
// NoteCreateRequestTags returns the canonical JSON key of every field on
...
func NoteCreateRequestTags() []string {
	return append([]string(nil), noteCreateRequestTags...)
}
...
// NotePatchRequestTags returns the canonical JSON key of every field on
...
// the source of truth; this accessor is for consumers that decline the generated
// handler and write their own bind step.
func NotePatchRequestTags() []string {
	return append([]string(nil), notePatchRequestTags...)
}
...
func (v *ValidNotePatchRequest) Body() (string, bool) {
...
// reads as "this note has none".
//
// That error is cheap because NoteQueryWithResponseEdges below is
...
func NewNoteResponse(e *Note) (*NoteResponse, error) {
	...
	docsEdge, err := e.Edges.DocsOrErr()
	if err != nil {
		return nil, err
	}
	r.Docs = make([]*DocSummary, 0, len(docsEdge))
	for _, v := range docsEdge {
		r.Docs = append(r.Docs, NewDocSummary(v))
	}
	return r, nil
}
...
//
// NoteClient.Get loads no edges and therefore cannot serve this response
// type. Wiring for Note must go through Query with this plan applied.
