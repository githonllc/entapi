...
// ============================================================================
// Query surface for Doc — filters, free-text search and the sort
...
// DocFilter is the structured query surface for Doc.
//
...
// docSortOptions maps an allowed sort key to ent's own order
// builder for that column. It is unexported: the mapping is reached through
// DocOrder, which checks the key first.
var docSortOptions = map[string]func(...sql.OrderTermOption) doc.OrderOption{}

// DocSortKeys is the sort allow-list: exactly the fields annotated
...
// DocOrder turns a list request into ent order options.
//
// The caller's string is checked against DocSortKeys and then thrown
...
func DocOrder(r entapi.ListRequest) ([]doc.OrderOption, error) {
	key, desc, err := r.SortKey(DocSortKeys, "")
	if err != nil {
		return nil, err
	}
	dir := sql.OrderAsc()
	...
	by, ok := docSortOptions[key]
	if !ok {
		return []doc.OrderOption{doc.ByID(sql.OrderAsc())}, nil
	}
	order := []doc.OrderOption{by(dir)}
	if key != "id" {
		order = append(order, doc.ByID(dir))
	}
	return order, nil
}
