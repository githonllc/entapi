...
func (aq *AccountQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("createexceptedent: check existence: %w", err)
	default:
		return true, nil
	}
}
...
//		Aggregate(createexceptedent.Count()).
//		Scan(ctx, &v)
...
func (aq *AccountQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range aq.inters {
		if inter == nil {
			return fmt.Errorf("createexceptedent: uninitialized interceptor (forgotten import createexceptedent/runtime?)")
		}
	...
	for _, f := range aq.ctx.Fields {
		if !account.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("createexceptedent: invalid field %q for query", f)}
		}
	}
	if aq.path != nil {
