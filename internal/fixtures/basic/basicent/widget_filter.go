...
// WidgetFilter is the typed query representation for Widget.
...
func (f *WidgetFilter) Predicates() []predicate.Widget {
	...
	entapi.AppendEach(&ps, f.ID, widget.IDEQ)
	entapi.AppendEach(&ps, f.IDNEQ, widget.IDNEQ)
	entapi.AppendEachSlice(&ps, f.IDIn, widget.IDIn)
	entapi.AppendEachSlice(&ps, f.IDNotIn, widget.IDNotIn)
	entapi.AppendEach(&ps, f.IDGT, widget.IDGT)
	entapi.AppendEach(&ps, f.IDGTE, widget.IDGTE)
	entapi.AppendEach(&ps, f.IDLT, widget.IDLT)
	entapi.AppendEach(&ps, f.IDLTE, widget.IDLTE)
	return ps
}

func parseWidgetIDQueryValue(raw, whole string) (uuid.UUID, error) {
...
// ParseWidgetQuery parses the complete URL query contract in sorted-key
...
func ParseWidgetQuery(q url.Values) (*WidgetFilter, entapi.ListRequest, error) {
	f := &WidgetFilter{}
	...
	for _, key := range keys {
		...
		if recognized {
			...
			case "_q":
				err = fmt.Errorf("%w: _q value %q is invalid because Widget has no Searchable fields", entapi.ErrValidation, reserved)
			}
			if err != nil {
				return nil, entapi.ListRequest{}, err
			}
		...
		case "id":
			for _, whole := range values {
				...
				switch op {
				case "eq":
					parsed, parseErr := parseWidgetIDQueryValue(raw, whole)
					if parseErr != nil {
				...
				case "in":
					...
					for _, part := range parts {
						parsed, parseErr := parseWidgetIDQueryValue(part, whole)
						if parseErr != nil {
							return nil, entapi.ListRequest{}, parseErr
						}
				...
				case "between":
					...
					lower, parseErr := parseWidgetIDQueryValue(parts[0], whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
					upper, parseErr := parseWidgetIDQueryValue(parts[1], whole)
					if parseErr != nil {
						return nil, entapi.ListRequest{}, parseErr
					}
...
// WidgetSortKeys is the complete sort allow-list. The primary key is
// naturally Sortable; every other key requires api.Sortable().
var WidgetSortKeys = []string{
	"id",
}

// WidgetOrder is the single sort allow-list seam.
func WidgetOrder(r entapi.ListRequest) ([]widget.OrderOption, error) {
	if len(r.Sort) == 0 {
		return []widget.OrderOption{widget.ByID(sql.OrderAsc())}, nil
	}
	orderBy := make([]widget.OrderOption, 0, len(r.Sort)+1)
	idPresent := false
	lastDir := sql.OrderAsc()
	for _, spec := range r.Sort {
		by, ok := widgetSortOptions[spec.Key]
		if !ok {
			return nil, fmt.Errorf("%w: cannot sort by %q; legal keys: %s", entapi.ErrValidation, spec.Key, strings.Join(WidgetSortKeys, ", "))
		}
		dir := sql.OrderAsc()
		if spec.Desc {
			dir = sql.OrderDesc()
	...
	if !idPresent {
		orderBy = append(orderBy, widget.ByID(lastDir))
	}
	return orderBy, nil
}
