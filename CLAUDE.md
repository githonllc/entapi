...
`README.md` and `README_zh.md` are parallel translations; changing the public API means editing both, and both carry the migration notes for every symbol this module has removed. **Each README is an entry point, not the reference**: the long form lives in `docs/GUIDE.md` and `docs/GUIDE_zh.md`, which are themselves a parallel pair. A rule that gains a reason, a source pointer or an edge case belongs in the GUIDE; the README carries the summary table and links to it. Migration notes are the deliberate exception — they stay in the READMEs, because the `doc.go` / `runtime/doc.go` comments point there. There are now **two** `doc.go`s: the root one is the generator's godoc quick start and the runtime migration pointer, `runtime/doc.go` is the runtime's. `docs/ARCHITECTURE.md` carries the module table and the PlantUML diagrams, and `.claude/skills/entapi/SKILL.md` documents downstream usage patterns (some of it describes a consumer project's interceptors, not this repo).
