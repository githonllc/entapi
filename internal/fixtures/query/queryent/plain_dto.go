...
// PlainCreateRequestTags returns the canonical JSON key of every field on
// PlainCreateRequest, in declaration order. Generated handlers compare raw
...
func PlainCreateRequestTags() []string {
	return append([]string(nil), plainCreateRequestTags...)
}
...
// PlainPatchRequestTags returns the canonical JSON key of every field on
// PlainPatchRequest, in declaration order. Generated handlers compare raw
// request keys against this data to reject unknown keys.
//
// Like Routes(), it returns a fresh copy on every call: slices are mutable, so
// exporting the variable would let any consumer mutate validation data that
...
// the source of truth; this accessor is for consumers that decline the generated
// handler and write their own bind step.
func PlainPatchRequestTags() []string {
	return append([]string(nil), plainPatchRequestTags...)
}
...
func (v *ValidPlainPatchRequest) Label() (string, bool) {
...
// NewPlainResponse return a not-loaded-edge error at run time. It fails
