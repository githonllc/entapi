...
// ============================================================================
// Query surface for Plain — filters, free-text search and the sort
...
// PlainFilter is the structured query surface for Plain.
//
...
// plainSortOptions maps an allowed sort key to ent's own order
// builder for that column. It is unexported: the mapping is reached through
// PlainOrder, which checks the key first.
var plainSortOptions = map[string]func(...sql.OrderTermOption) plain.OrderOption{}

// PlainSortKeys is the sort allow-list: exactly the fields annotated
...
// PlainOrder turns a list request into ent order options.
//
// The caller's string is checked against PlainSortKeys and then thrown
...
func PlainOrder(r entdomain.ListRequest) ([]plain.OrderOption, error) {
	key, desc, err := r.SortKey(PlainSortKeys, "")
	if err != nil {
		return nil, err
	}
	by, ok := plainSortOptions[key]
	if !ok {
		// Only reachable when no sort was requested and no default applies.
		return nil, nil
	}
	if desc {
		return []plain.OrderOption{by(sql.OrderDesc())}, nil
	}
	return []plain.OrderOption{by(sql.OrderAsc())}, nil
}
